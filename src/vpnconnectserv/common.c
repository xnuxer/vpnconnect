 ...
 */
int
vpnconnect_vsntprintf (LPTSTR str, size_t size, LPCTSTR format, va_list arglist)
{
  int len = -1;
  if (size > 0)
    {
...
int
vpnconnect_sntprintf (LPTSTR str, size_t size, LPCTSTR format, ...)
{
  va_list arglist;
  int len = -1;
  if (size > 0)
    {
      ...
      len = vpnconnect_vsntprintf (str, size, format, arglist);
      va_end (arglist);
    }
  return len;
}
...
GetOpenvpnSettings (settings_t *s)
{
  ...
  if (error != ERROR_SUCCESS)
  {
    vpnconnect_sntprintf(s->ovpn_admin_group, _countof(s->ovpn_admin_group), OVPN_ADMIN_GROUP);
...
GetLastErrorText ()
{
  ...
  if (len == 0 || (long) _countof (buf) < (long) len + 14)
  ...
  else
    {
      ...
      vpnconnect_sntprintf (buf, _countof (buf), TEXT("%s (0x%x)"), tmp, GetLastError());
    }
...
MsgToEventLog (DWORD flags, LPCTSTR format, ...)
{
  ...
  if (hEventSource != NULL)
    {
      vpnconnect_sntprintf (msg[0], _countof (msg[0]),
                         TEXT("%s%s: %s"), APPNAME,
                         (flags & MSG_FLAGS_ERROR) ? TEXT(" error") : TEXT(""), err_msg);

      va_start (arglist, format);
      vpnconnect_vsntprintf (msg[1], _countof (msg[1]), format, arglist);
      va_end (arglist);
