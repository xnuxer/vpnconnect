...
/*
 * This program allows one or more VPNConnect.processes to be started
...
vpnconnect_service_t automatic_service = {
  automatic,
...
ServiceStartAutomatic (DWORD dwArgc, LPTSTR *lpszArgv)
{
  ...
  /*
   * Instantiate an VPNConnect.process for each configuration
   * file found.
   */
  {
    ...
    vpnconnect_sntprintf (find_string, MAX_PATH, TEXT("%s\\*"), settings.config_dir);
      ...
      if (match (&find_obj, settings.ext_string))
        {
          ...
          vpnconnect_sntprintf (log_path, _countof (log_path),
                             TEXT("%s\\%s"), settings.log_dir, log_file);

          /* construct command line */
          vpnconnect_sntprintf (command_line, _countof (command_line), TEXT(PACKAGE " --service %s 1 --config \"%s\""),
                      EXIT_EVENT_NAME,
                      find_obj.cFileName);
          ...
          /* create an VPNConnect.process for one config file */
