...
#define ERROR_VPNCONNECT_STARTUP  0x20000000
...
vpnconnect_service_t interactive_service = {
  interactive,
...
ReturnError (HANDLE pipe, DWORD error, LPCWSTR func, DWORD count, LPHANDLE events)
{
  ...
  if (error != ERROR_VPNCONNECT_STARTUP)
    {
...
ReturnOpenvpnOutput (HANDLE pipe, HANDLE ovpn_output, DWORD count, LPHANDLE events)
{
  ...
  ReturnError (pipe, ERROR_VPNCONNECT_STARTUP, wide_output, count, events);
  free (wide_output);
}
...
RunOpenvpn (LPVOID p)
{
  ...
  /* VPNConnect.process DACL entry for access by service and user */
  ...
  /* Set owner and DACL of VPNConnect.security descriptor */
  ...
  /* use /dev/null for stdout of vpnconnect (client should use --log for output) */
  ...
  vpnconnect_sntprintf (ovpn_pipe_name, _countof (ovpn_pipe_name),
    TEXT("\\\\.\\pipe\\vpnconnect\\service_%lu"), GetCurrentThreadId ());
  ...
  vpnconnect_sntprintf (cmdline, cmdline_size, L"vpnconnect %s --msg-channel %lu",
  ...
  if (exit_code == STILL_ACTIVE)
  ...
  else if (exit_code != 0)
    {
      ...
      int len = _snwprintf (buf, _countof (buf),
                         L"VPNConnect exited with error: exit code = %lu", exit_code);
      buf[_countof (buf) - 1] =  L'\0';
      ReturnError (pipe, ERROR_VPNCONNECT_STARTUP, buf, 1, &exit_event);
    }
  Undo (&undo_lists);
...
CreateClientPipeInstance (VOID)
{
  ...
  pipe = CreateNamedPipe (TEXT("\\\\.\\pipe\\vpnconnect\\service"), flags,
...
ServiceStartInteractive (DWORD dwArgc, LPTSTR *lpszArgv)
{
  ...
  /* Read info from registry in key HKLM\SOFTWARE\VPNConnect.*/
