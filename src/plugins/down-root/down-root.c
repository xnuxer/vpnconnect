...
/*
 * VPNConnect.plugin module to do privileged down-script execution.
 */
...
vpnconnect_plugin_open_v1 (unsigned int *type_mask, const char *argv[], const char *envp[])
{
     ...
     */
    *type_mask = VPNCONNECT_PLUGIN_MASK (VPNCONNECT_PLUGIN_UP) | VPNCONNECT_PLUGIN_MASK (VPNCONNECT_PLUGIN_DOWN);
...
vpnconnect_plugin_func_v1 (vpnconnect_plugin_handle_t handle, const int type, const char *argv[], const char *envp[])
{
    ...
    if (type == VPNCONNECT_PLUGIN_UP && context->foreground_fd == -1) /* fork off a process to hold onto root */
    {
        pid_t pid;
        int fd[2];
    ...
    else if (type == VPNCONNECT_PLUGIN_DOWN && context->foreground_fd >= 0)
    {
...
VPNCONNECT_EXPORT void
vpnconnect_plugin_abort_v1 (vpnconnect_plugin_handle_t handle)
{
    struct down_root_context *context = (struct down_root_context *) handle;
