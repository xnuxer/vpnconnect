 ...
 *             packet compression.
 *
 *  Copyright (C) 2002-2013 VPNConnect.Technologies, Inc. <sale@vpnconnect.net>
...
static const char usage_message[] =
  ...
  "                  directives for future VPNConnect.versions to be ignored.\n"
  ...
  "                  the username and the password are omitted VPNConnect.will prompt\n"
  ...
  "                  in an vpnconnect temporary file in [directory]. Peer cert is \n"
...
init_options (struct options *o, const bool init_gc)
{
  ...
  o->ce.local_port = o->ce.remote_port = VPNCONNECT_PORT;
  o->verbosity = 1;
			   ...
			   struct gc_arena *gc)
{
  if (server && port)
	...
	ho->auth_retry = PAR_ALL;
      ho->http_version = "1.0";
      ho->user_agent = "VPNConnect-Autoproxy/1.0";
      return ho;
    }
  else
    return NULL;
...
options_postprocess_verify_ce (const struct options *options, const struct connection_entry *ce)
{
  ...
  if (options->tls_server || options->tls_client)
     ...
     if (options->cryptoapi_cert)
	{
	  ...
	  msg(M_USAGE, "Parameter --pkcs12 cannot be used with the PolarSSL version version of VPNConnect.");
#else
          if (options->ca_path)
	  ...
	  if (!(options->ca_file))
	    msg(M_USAGE, "You must define CA file (--ca)");
          if (options->ca_path)
            msg(M_USAGE, "Parameter --capath cannot be used with the PolarSSL version version of VPNConnect.");
#else
	  if ((!(options->ca_file)) && (!(options->ca_path)))
	    msg(M_USAGE, "You must define CA file (--ca) or CA path (--capath)");
#endif
...
usage (void)
{
  ...
  vpnconnect_exit (VPNCONNECT_EXIT_STATUS_USAGE); /* exit point */
}
...
usage_version (void)
{
  ...
  msg (M_INFO|M_NOPREFIX, "Copyright (C) 2002-2010 OpenVPN.Technologies, Inc. <sale@vpnconnect.net>");
#ifndef ENABLE_SMALL
		 ...
		 bool in_chroot)
{
  ...
  *script = new_script;
...
#ifndef ENABLE_SMALL
  {
    ...
    vpnconnect_snprintf (script_name, sizeof(script_name),
                      "--%s script", type);
	    ...
	    struct env_set *es)
{
  ...
  if (streq (p[0], "version") && !p[1])
  ...
  else if (streq (p[0], "show-gateway") && !p[2])
    {
      ...
      vpnconnect_exit (VPNCONNECT_EXIT_STATUS_GOOD); /* exit point */
    }
#endif
#if 0
...
/*      VERIFY_PERMISSION (OPT_P_MTU); */
    ...
    {
      VERIFY_PERMISSION (OPT_P_GENERAL|OPT_P_CONNECTION);
      msg (M_WARN, "DEPRECATED OPTION: http-proxy-timeout: In VPNConnect.2.4 the timeout until a connection to a "
  ...
  else if (streq (p[0], "win-sys") && p[1] && !p[2])
    {
      VERIFY_PERMISSION (OPT_P_GENERAL);
      if (streq (p[1], "env"))
	msg (M_INFO, "NOTE: --win-sys env is default from VPNConnect.v2.3.	 "
  ...
  else if (streq (p[0], "block-outside-dns") && !p[1])
    {
      ...
      *
      * (if --register-dns is set, vpnconnect needs to call itself in a
      ...
      if (strlen (p[1]))
        {
          ...
          /*
           * Enable legacy vpnconnect format for DNs that have not been converted
