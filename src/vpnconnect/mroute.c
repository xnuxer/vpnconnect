...
{
  unsigned int ret = 0;
  if (BLEN (buf) >= (int) sizeof (struct vpnconnect_arp))
    {
      const struct vpnconnect_arp *arp = (const struct vpnconnect_arp *) BPTR (buf);
			  ...
			  const struct buffer *buf)
{
  ...
  if (BLEN (buf) >= 1)
    {
      switch (VPNCONNECT_IPH_GET_VER (*BPTR(buf)))
	{
	case 4:
	  if (BLEN (buf) >= (int) sizeof (struct vpnconnect_iphdr))
	    {
	      const struct vpnconnect_iphdr *ip = (const struct vpnconnect_iphdr *) BPTR (buf);
		...
		ret |= MROUTE_EXTRACT_MCAST;
	      ...
	      /* IGMP message? */
	      if (ip->protocol == VPNCONNECT_IPPROTO_IGMP)
	...
	case 6:
	  if (BLEN (buf) >= (int) sizeof (struct vpnconnect_ipv6hdr))
	    {
	      const struct vpnconnect_ipv6hdr *ipv6 = (const struct vpnconnect_ipv6hdr *) BPTR (buf);
	...
	default:
	    msg (M_WARN, "IP packet with unknown IP version=%d seen",
	                 VPNCONNECT_IPH_GET_VER (*BPTR(buf)));
	}
    }
  return ret;
}
...
{
  unsigned int ret = 0;
  if (BLEN (buf) >= (int) sizeof (struct vpnconnect_ethhdr))
    {
      const struct vpnconnect_ethhdr *eth = (const struct vpnconnect_ethhdr *) BPTR (buf);
      if (src)
	{
	...
	{
	  struct buffer b = *buf;
	  if (buf_advance (&b, sizeof (struct vpnconnect_ethhdr)))
	    {
	      switch (ntohs (eth->proto))
		{
		case VPNCONNECT_ETH_P_IPV4:
		  ret |= (mroute_extract_addr_ipv4 (esrc, edest, &b) << MROUTE_SEC_SHIFT);
		  break;
		case VPNCONNECT_ETH_P_ARP:
...
/*
 * Translate a struct vpnconnect_sockaddr (osaddr)
 * to a struct mroute_addr (addr).
 */
bool mroute_extract_vpnconnect_sockaddr (struct mroute_addr *addr,
				      const struct vpnconnect_sockaddr *osaddr,
				      bool use_port)
{
