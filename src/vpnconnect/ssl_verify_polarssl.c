 ...
 */
void
x509_setenv (struct env_set *es, int cert_depth, vpnconnect_x509_cert_t *cert)
{
  int i;
  unsigned char c;
  ...
  while( name != NULL )
	...
	{
	  vpnconnect_snprintf (name_expand, sizeof(name_expand), "X509_%d_%s",
	      cert_depth, shortname);
	}
      else
	{
	  vpnconnect_snprintf (name_expand, sizeof(name_expand), "X509_%d_\?\?",
	      cert_depth);
	}
