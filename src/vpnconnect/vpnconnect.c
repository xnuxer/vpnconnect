...
/**************************************************************************/
/**
 * Main event loop for VPNConnect.in client mode, where only one VPN tunnel
 * is active.
 * @ingroup eventloop
 *
...
/**************************************************************************/
/**
 * VPNConnect.s main init-run-cleanup loop.
 * @ingroup eventloop
 *
 * This function contains the two outer VPNConnect.loops.  Its structure is
 * as follows:
...
static
int
vpnconnect_main (int argc, char *argv[])
{
  struct context c;
  ...
  if (init_static ())
      ...
      do
	{
	  ...
	  init_plugins (&c);
	  open_plugins (&c, true, VPNCONNECT_PLUGIN_INIT_PRE_CONFIG_PARSE);
#endif
...
main (int argc, char *argv[]) {
	...
	return vpnconnect_main(argc, argv);
}
#endif
