 ...
 *
 * Also, grep for VALGRIND_MAKE_READABLE in the VPNConnect.source.
 * Because valgrind thinks that some of the data passed from
 * OpenSSL back to VPNConnect.is tainted due to being sourced
 * from uninitialized data, we need to untaint it before use --
 * otherwise we will get a lot of useless warnings.
 *
 *   valgrind --tool=memcheck --error-limit=no --suppressions=debug/valgrind-suppress --gen-suppressions=yes ./vpnconnect ...
 */
 ...
 * use this command prior to running vpnconnect:
 *
...
#define vpnconnect_dmalloc(file, line, size) dmalloc_malloc((file), (line), (size), DMALLOC_FUNC_MALLOC, 0, 0)
...
#if 0
#undef malloc
#define malloc(size) vpnconnect_dmalloc("logfile", x_msg_line_num, (size))
#endif
