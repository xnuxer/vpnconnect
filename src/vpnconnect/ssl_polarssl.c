    ...
    unsigned char *sig )
{
  ...
  if (vpnconnect_base64_encode (sig, asn_len + hashlen, &in_b64) <= 0)
    {
  ...
  if ( vpnconnect_base64_decode (out_b64, sig, ctx->signature_length) !=
...
/**
 * Convert an VPNConnect.tls-version variable to PolarSSl format (i.e. a major and
    ...
    const struct tls_root_ctx *ssl_ctx, bool is_server, struct tls_session *session)
{
  ...
  if (polar_ok(ssl_init(ks_ssl->ctx)))
	...
	ssl_set_ciphersuites (ks_ssl->ctx, ssl_ctx->allowed_ciphers);
      ...
      /* Disable record splitting (for now).  VPNConnect.assumes records are sent
       * unfragmented, and changing that will require thorough review and
       * testing.  Since VPNConnect.is not susceptible to BEAST, we can just
...
print_details (struct key_state_ssl * ks_ssl, const char *prefix)
{
  ...
  s1[0] = s2[0] = 0;
  vpnconnect_snprintf (s1, sizeof (s1), "%s %s, cipher %s",
		    prefix,
  ...
  if (cert != NULL)
    {
      vpnconnect_snprintf (s2, sizeof (s2), ", %zu bit key", pk_get_size(&cert->pk));
    }
