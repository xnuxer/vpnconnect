         ...
         volatile int *signal_received)
{
  struct addrinfo *ai;
  int status;
  status = vpnconnect_getaddrinfo (flags & ~GETADDR_HOST_ORDER, hostname, NULL,
		    ...
		    const int flags)
{
  ...
  status = vpnconnect_getaddrinfo (flags, hostname, servname,
 ...
 */
int
vpnconnect_getaddrinfo (unsigned int flags,
                     const char *hostname,
                     const char *servname,
                     int resolve_retry_seconds,
                     volatile int *signal_received,
  ...
  if (status != 0) /* parse as numeric address failed? */
    {
	  ...
	  res_init ();
          ...
          vpnconnect_sleep (fail_wait_interval);
        }
 ...
 */
int
vpnconnect_inet_aton (const char *dotted_quad, struct in_addr *addr)
{
...
ip_addr_dotted_quad_safe (const char *dotted_quad)
{
  ...
  {
    struct in_addr a;
    return vpnconnect_inet_aton (dotted_quad, &a) == OIA_IP;
  }
}
...
#if 0 /* For debugging only, test the effect of accept() failures */
 {
  ...
  /* only valid if we have remote_len_af!=0 */
  else if (remote_len_af && remote_len != remote_len_af)
    {
      ...
      vpnconnect_close_socket (new_sd);
...
{
  struct gc_arena gc = gc_new ();
  /* struct vpnconnect_sockaddr *remote = &act->dest; */
  struct vpnconnect_sockaddr remote_verify = act->dest;
  ...
  while (true)
      ...
      if (socket_defined (new_sd))
	{
          ...
          if(remote_dynamic)
              vpnconnect_getaddrinfo(0, remote_dynamic, NULL, 1, NULL,
          ...
          if(ai && !addrlist_match(&remote_verify, ai))
            {
              msg (M_WARN,
                   ...
                   print_link_socket_actual (act, &gc));
              if (vpnconnect_close_socket (new_sd))
  ...
  if (!nowait && vpnconnect_close_socket (sd))
             ...
             bool ipv6only)
{
  ...
  if (bind (sd, cur->ai_addr, cur->ai_addrlen))
    {
      const int errnum = vpnconnect_errno ();
...
int
vpnconnect_connect (socket_descriptor_t sd,
                ...
                struct signal_info* sig_info)
{
  ...
  status = vpnconnect_connect (*sd, dest, connect_timeout, &sig_info->signal_received);
...
resolve_bind_local (struct link_socket *sock, const sa_family_t af)
{
  ...
  if (!sock->info.lsa->bind_local)
	...
	flags |= GETADDR_DATAGRAM;
      ...
      if (status)
	status = vpnconnect_getaddrinfo(flags, sock->local_host, sock->local_port, 0,
  ...
  if (!sock->info.lsa->remote_list)
      ...
      if (sock->remote_host)
	{
	  ...
	  status = get_cached_dns_entry (sock->dns_cache,
					 ...
					 flags, &ai);
	  if (status)
	    status = vpnconnect_getaddrinfo (flags, sock->remote_host, sock->remote_port,
  ...
  if (sock->http_proxy)
    {
      ...
      /* the VPNConnect.server we will use the proxy to connect to */
		   ...
		   const char *remote_dynamic, volatile int *signal_received)
{
  ...
  if (sock->info.proto == PROTO_TCP_SERVER) {
...
#ifdef HAVE_GETSOCKNAME
    {
      ...
      struct vpnconnect_sockaddr local_addr;
...
phase2_tcp_client (struct link_socket *sock, struct signal_info *sig_info)
{
  ...
  do {
    ...
    if (proxy_retry)
      {
	vpnconnect_close_socket (sock->sd);
...
link_socket_close (struct link_socket *sock)
{
  if (sock)
      ...
      if (socket_defined (sock->sd))
	{
	  ...
	  if (!gremlin)
	    {
	      ...
	      if (vpnconnect_close_socket (sock->sd))
      ...
      if (socket_defined (sock->ctrl_sd))
	{
	  if (vpnconnect_close_socket (sock->ctrl_sd))
				  ...
				  struct env_set *es)
{
  ...
  if (plugin_defined (info->plugins, VPNCONNECT_PLUGIN_IPCHANGE))
    {
      struct argv argv = argv_new ();
      ipchange_fmt (false, &argv, info, &gc);
      if (plugin_call (info->plugins, VPNCONNECT_PLUGIN_IPCHANGE, &argv, NULL, es) != VPNCONNECT_PLUGIN_FUNC_SUCCESS)
  ...
  if (info->ipchange_command)
    {
      ...
      vpnconnect_run_script (&argv, es, 0, "--ipchange");
      argv_reset (&argv);
    }
		  ...
		  int length_added)
{
  ...
  if (sb->len < 0 && sb->buf.len >= (int) sizeof (packet_size_type))
	...
	{
	  if (!is_vpnconnect_protocol (&sb->buf))
	    {
	      msg (D_STREAM_ERRORS, "Non-VPNConnect.client protocol detected");
...
void
setenv_sockaddr (struct env_set *es, const char *name_prefix, const struct vpnconnect_sockaddr *addr, const unsigned int flags)
{
  char name_buf[256];
  ...
  switch(addr->addr.sa.sa_family)
    ...
    case AF_INET:
      if (flags & SA_IP_PORT)
	vpnconnect_snprintf (name_buf, sizeof (name_buf), "%s_ip", name_prefix);
      else
	vpnconnect_snprintf (name_buf, sizeof (name_buf), "%s", name_prefix);

      setenv_str (es, name_buf, inet_ntoa (addr->addr.in4.sin_addr));

      if ((flags & SA_IP_PORT) && addr->addr.in4.sin_port)
	{
	  vpnconnect_snprintf (name_buf, sizeof (name_buf), "%s_port", name_prefix);
      ...
      if ( IN6_IS_ADDR_V4MAPPED( &addr->addr.in6.sin6_addr ))
	{
	  ...
	  vpnconnect_snprintf (name_buf, sizeof (name_buf), "%s_ip", name_prefix);
	  vpnconnect_snprintf (buf, sizeof(buf), "%s", inet_ntoa(ia) );
	}
      else
        {
	  vpnconnect_snprintf (name_buf, sizeof (name_buf), "%s_ip6", name_prefix);
 ...
 * checking.
 *
 * IPv6 and IPv4 protocols are comptabile bu. VPNConnect
...
struct vpnconnect_in6_pktinfo
{
  struct cmsghdr cmsghdr;
  struct in6_pktinfo pi6;
};

union vpnconnect_pktinfo {
	struct vpnconnect_in4_pktinfo msgpi4;
	struct vpnconnect_in6_pktinfo msgpi6;
};
#pragma pack()
...
#error ENABLE_IP_PKTINFO is set without IP_PKTINFO xor IP_RECVDSTADDR (fix syshead.h)
#endif
	  && cmsg->cmsg_len >= sizeof (struct vpnconnect_in4_pktinfo))
	{
#ifdef IP_PKTINFO
...
#error ENABLE_IP_PKTINFO is set without IP_PKTINFO xor IP_RECVDSTADDR (fix syshead.h)
	  ...
	  && cmsg->cmsg_len >= sizeof (struct vpnconnect_in6_pktinfo))
	{
    ...
    case AF_INET:
      {
        ...
        mesg.msg_flags = 0;
#ifdef HAVE_IN_PKTINFO
        mesg.msg_controllen = sizeof (struct vpnconnect_in4_pktinfo);
        cmsg = CMSG_FIRSTHDR (&mesg);
        cmsg->cmsg_len = sizeof (struct vpnconnect_in4_pktinfo);
        cmsg->cmsg_level = SOL_IP;
        cmsg->cmsg_type = IP_PKTINFO;
    ...
    case AF_INET6:
      {
        ...
        mesg.msg_control = &opi;
        mesg.msg_controllen = sizeof (struct vpnconnect_in6_pktinfo);
        mesg.msg_flags = 0;
        cmsg = CMSG_FIRSTHDR (&mesg);
        cmsg->cmsg_len = sizeof (struct vpnconnect_in6_pktinfo);
        cmsg->cmsg_level = IPPROTO_IPV6;
