    ...
    int line)
{
  ...
  if (!vpnconnect_snprintf(prefix, sizeof(prefix), "%s:%d", func, line))
  ...
  if (!rand_initialised)
    {
       ...
       */
      buf_printf(&pers_string, "VPNConnect %0u %p %s", platform_getpid(), &cd_ctx, time_string(0, 0, 0, &gc));
...
cipher_kt_name (const cipher_info_t *cipher_kt)
{
  ...
  return translate_cipher_name_to_vpnconnect(cipher_kt->name);
}
...
cipher_kt_tag_size (const cipher_info_t *cipher_kt)
{
  ...
  if (cipher_kt && cipher_kt_mode_aead(cipher_kt))
    return VPNCONNECT_AEAD_TAG_LENGTH;
#endif
  return 0;
}
...
{
  return cipher && cipher_kt_mode(cipher) == VPNCONNECT_MODE_CBC;
}

bool
cipher_kt_mode_ofb_cfb(const cipher_kt_t *cipher)
{
  return cipher && (cipher_kt_mode(cipher) == VPNCONNECT_MODE_OFB ||
	  cipher_kt_mode(cipher) == VPNCONNECT_MODE_CFB);
}

bool
cipher_kt_mode_aead(const cipher_kt_t *cipher)
{
  return cipher && cipher_kt_mode(cipher) == VPNCONNECT_MODE_GCM;
}
