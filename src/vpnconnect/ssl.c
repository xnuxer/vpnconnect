...
tls_session_user_pass_enabled(struct tls_session *session)
{
  return (session->opt->auth_user_pass_verify_script
        || plugin_defined (session->opt->plugins, VPNCONNECT_PLUGIN_AUTH_USER_PASS_VERIFY)
#ifdef MANAGEMENT_DEF_AUTH
  ...
  if (session->tls_auth.key_ctx_bi.encrypt.hmac)
    {
      ...
      vpnconnect_encrypt (buf, null, &session->tls_auth);
		   ...
		   const struct link_socket_actual *from)
{
  ...
  if (co->key_ctx_bi.decrypt.hmac)
      ...
      if (!swap_hmac (buf, co, true))
	{
      ...
      vpnconnect_decrypt (buf, null, co, NULL, BPTR (buf));
      if (!buf->len)
	{
	  msg (D_TLS_ERRORS,
...
static void
vpnconnect_PRF (const uint8_t *secret,
	     int secret_len,
	     const char *label,
			...
			bool server)
{
  ...
  vpnconnect_PRF (key_src->client.pre_master,
  ...
  if (cipher_kt_mode_aead (cipher_kt))
    {
      ...
      ASSERT (cipher_kt_iv_size (cipher_kt) >= VPNCONNECT_AEAD_MIN_IV_LEN);
      impl_iv_len = cipher_kt_iv_size (cipher_kt) - sizeof (packet_id_type);
      ASSERT (impl_iv_len <= VPNCONNECT_MAX_IV_LENGTH);
...
key_method_1_write (struct buffer *buf, struct tls_session *session)
{
  ...
  init_key_ctx (&ks->crypto_options.key_ctx_bi.encrypt, &key,
		&session->opt->key_type, VPNCONNECT_OP_ENCRYPT,
		"Data Channel Encrypt");
  CLEAR (key);
...
key_method_1_read (struct buffer *buf, struct tls_session *session)
{
  ...
  init_key_ctx (&ks->crypto_options.key_ctx_bi.decrypt, &key,
		&session->opt->key_type, VPNCONNECT_OP_DECRYPT,
		"Data Channel Decrypt");
  CLEAR (key);
...
key_method_2_read (struct buffer *buf, struct tls_multi *multi, struct tls_session *session)
{
  ...
  /*
   * Call VPNCONNECT_PLUGIN_TLS_FINAL plugin if defined, for final
   * veto opportunity over authentication decision.
   */
  if (ks->authenticated && plugin_defined (session->opt->plugins, VPNCONNECT_PLUGIN_TLS_FINAL))
    {
      key_state_export_keying_material(&ks->ks_ssl, session);

      if (plugin_call (session->opt->plugins, VPNCONNECT_PLUGIN_TLS_FINAL, NULL, NULL, session->opt->es) != VPNCONNECT_PLUGIN_FUNC_SUCCESS)
	ks->authenticated = false;
 ...
 *
 * Note that vpnconnect only allows one active session at a time,
	  ...
	  for (i = 0; i < KEY_SCAN_SIZE; ++i)
	    {
	      ...
	      /*
	       * This is the basic test of TLS state compatibility between a local VPNConnect.
	       ...
	       * VPNConnect.instance has no memory of such a negotiation.
	       *
...
/*
 * Dump a human-readable rendition of an vpnconnect packet
