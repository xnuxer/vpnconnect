...
encrypt_sign (struct context *c, bool comp_frag)
{
  ...
  if (c->c2.tls_multi)
    {
       ...
       * packet before vpnconnect_encrypt(), so we can authenticate the opcode too.
       */
  ...
  vpnconnect_encrypt (&c->c2.buf, b->encrypt_buf, co);
...
process_incoming_link_part1 (struct context *c, struct link_socket_info *lsi, bool floated)
{
  ...
  if (c->c2.buf.len > 0)
	...
	c->c2.buf.len = 0;
      ...
      decrypt_status = vpnconnect_decrypt (&c->c2.buf, c->c2.buffers->decrypt_buf,
...
process_incoming_link_part2 (struct context *c, struct link_socket_info *lsi, const uint8_t *orig_buf)
{
  if (c->c2.buf.len > 0)
      ...
      if (c->c2.buf.len > 0)
	{
      ...
      /* Did we just receive an vpnconnect ping packet? */
