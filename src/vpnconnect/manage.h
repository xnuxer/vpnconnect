...
struct log_entry
{
  ...
  struct vpnconnect_sockaddr local_sock;
  struct vpnconnect_sockaddr remote_sock;
...
# define MCF_SERVER (1<<0) /* is VPNConnect.being run as a server? */
  unsigned int flags;
...
struct man_connection {
  ...
  struct vpnconnect_sockaddr remote;
...
/*
 * VPNConnect.tells the management layer what state it's in
 */

/* client/server states */
#define VPNCONNECT_STATE_INITIAL       0  /* Initial, undefined state */
#define VPNCONNECT_STATE_CONNECTING    1  /* Management interface has been initialized */
#define VPNCONNECT_STATE_ASSIGN_IP     2  /* Assigning IP address to virtual network interface */
#define VPNCONNECT_STATE_ADD_ROUTES    3  /* Adding routes to system */
#define VPNCONNECT_STATE_CONNECTED     4  /* Initialization sequence completed */
#define VPNCONNECT_STATE_RECONNECTING  5  /* Restart */
#define VPNCONNECT_STATE_EXITING       6  /* Exit */

/* client-only states */
#define VPNCONNECT_STATE_WAIT          7  /* Waiting for initial response from server */
#define VPNCONNECT_STATE_AUTH          8  /* Authenticating with server */
#define VPNCONNECT_STATE_GET_CONFIG    9  /* Downloading configuration from server */
#define VPNCONNECT_STATE_RESOLVE       10 /* DNS lookup */
#define VPNCONNECT_STATE_TCP_CONNECT   11 /* Connecting to TCP server */

#define VPNCONNECT_STATE_CLIENT_BASE   7  /* Base index of client-only states */
...
void management_set_state (struct management *man,
			   ...
			   const char *detail,
                           ...
                           const struct vpnconnect_sockaddr *local_addr,
                           const struct vpnconnect_sockaddr *remote_addr);

/*
 * The management object keeps track of VPNConnect --echo
 * parameters.
 */
void management_echo (struct management *man, const char *string, const bool pull);

/*
 * VPNConnect.calls here to indicate a password failure
 */
