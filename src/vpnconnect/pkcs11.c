...
static
unsigned
_pkcs11_msg_pkcs112vpnconnect (
	const unsigned flags
) {
	unsigned vpnconnect_flags;

	switch (flags) {
		case PKCS11H_LOG_DEBUG2:
			vpnconnect_flags = D_PKCS11_DEBUG;
		break;
		case PKCS11H_LOG_DEBUG1:
			vpnconnect_flags = D_SHOW_PKCS11;
		break;
		case PKCS11H_LOG_INFO:
			vpnconnect_flags = M_INFO;
		break;
		case PKCS11H_LOG_WARN:
			vpnconnect_flags = M_WARN;
		break;
		case PKCS11H_LOG_ERROR:
			vpnconnect_flags = M_FATAL;
		break;
		default:
			vpnconnect_flags = M_FATAL;
		break;
	...
	return vpnconnect_flags;
}

static
unsigned
_pkcs11_msg_vpnconnect2pkcs11 (
	const unsigned flags
) {
...
_pkcs11_vpnconnect_log (
	...
	msg (_pkcs11_msg_pkcs112vpnconnect (flags), "%s", Buffer);
}

static
PKCS11H_BOOL
_pkcs11_vpnconnect_token_prompt (
	void * const global_data,
...
_pkcs11_vpnconnect_pin_prompt (
	...
	vpnconnect_snprintf (prompt, sizeof (prompt), "%s token", token->label);
...
pkcs11_initialize (
	...
	if ((rv = pkcs11h_setLogHook (_pkcs11_vpnconnect_log, NULL)) != CKR_OK) {
		msg (M_FATAL, "PKCS#11: Cannot set hooks %ld-'%s'", rv, pkcs11h_getMessage (rv));
		goto cleanup;
	}

	pkcs11h_setLogLevel (_pkcs11_msg_vpnconnect2pkcs11 (get_debug_level ()));
	...
	if ((rv = pkcs11h_setTokenPromptHook (_pkcs11_vpnconnect_token_prompt, NULL)) != CKR_OK) {
		msg (M_FATAL, "PKCS#11: Cannot set hooks %ld-'%s'", rv, pkcs11h_getMessage (rv));
		goto cleanup;
	}

	if ((rv = pkcs11h_setPINPromptHook (_pkcs11_vpnconnect_pin_prompt, NULL)) != CKR_OK) {
		msg (M_FATAL, "PKCS#11: Cannot set hooks %ld-'%s'", rv, pkcs11h_getMessage (rv));
		goto cleanup;
	}
...
pkcs11_management_id_get (
	...
	*base64 = NULL;
	...
	if (vpnconnect_base64_encode (certificate_blob, certificate_blob_size, &internal_base64) == -1) {
...
static
PKCS11H_BOOL
_pkcs11_vpnconnect_show_pkcs11_ids_pin_prompt (
	void * const global_data,
	void * const user_data,
	const pkcs11h_token_id_t token,
	const unsigned retry,
...
show_pkcs11_ids (
	...
	if ((rv = pkcs11h_setPINPromptHook (_pkcs11_vpnconnect_show_pkcs11_ids_pin_prompt, NULL)) != CKR_OK) {
