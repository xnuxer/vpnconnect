...
char *
backend_x509_get_serial (vpnconnect_x509_cert_t *cert, struct gc_arena *gc)
{
  ASN1_INTEGER *asn1_i;
  BIGNUM *bignum;
...
char *
backend_x509_get_serial_hex (vpnconnect_x509_cert_t *cert, struct gc_arena *gc)
{
...
do_setenv_x509 (struct env_set *es, const char *name, char *value, int depth)
{
  ...
  vpnconnect_snprintf (name_expand, name_expand_size, "X509_%d_%s", depth, name);
 ...
 */
void
x509_setenv (struct env_set *es, int cert_depth, vpnconnect_x509_cert_t *peer_cert)
{
  int i, n;
  int fn_nid;
  ASN1_OBJECT *fn;
  ...
  for (i = 0; i < n; ++i)
	...
	continue;
      ...
      name_expand = (char *) malloc (name_expand_size);
      check_malloc_return (name_expand);
      vpnconnect_snprintf (name_expand, name_expand_size, "X509_%d_%s", cert_depth,
	  objbuf);
...
result_t
x509_verify_ns_cert_type(const vpnconnect_x509_cert_t *peer_cert, const int usage)
{
