  ...
  if (plugin_defined (plugins, plugin_type))
    {
      ...
      if (plugin_call (plugins, plugin_type, &argv, NULL, es) != VPNCONNECT_PLUGIN_FUNC_SUCCESS)
  ...
  if (command)
    {
      ...
      vpnconnect_run_script (&argv, es, S_FATAL, "--up/--down");
      argv_reset (&argv);
    }
...
/*
 * Wrapper around vpnconnect_execve
 */
bool
vpnconnect_execve_check (const struct argv *a, const struct env_set *es, const unsigned int flags, const char *error_message)
{
  struct gc_arena gc = gc_new ();
  const int stat = vpnconnect_execve (a, es, flags);
  int ret = false;
...
bool
vpnconnect_execve_allowed (const unsigned int flags)
{
  if (flags & S_SCRIPT)
 ...
 */
int
vpnconnect_execve (const struct argv *a, const struct env_set *es, const unsigned int flags)
{
  struct gc_arena gc = gc_new ();
  int ret = -1;
  ...
  if (a && a->argv[0])
    {
      ...
      if (vpnconnect_execve_allowed (flags))
	{
          ...
          if (pid == (pid_t)0) /* child side */
            ...
            msg (M_ERR, "vpnconnect_execve: unable to fork");
          else /* parent side */
            {
...
#else
      msg (M_WARN, "vpnconnect_execve: execve function not available");
#endif
    }
  else
    {
      msg (M_FATAL, "vpnconnect_execve: called with empty argv");
    }
 ...
 */
int
vpnconnect_popen (const struct argv *a,  const struct env_set *es)
{
  struct gc_arena gc = gc_new ();
  int ret = -1;
  static bool warn_shown = false;
      ...
      if (script_security >= SSEC_BUILT_IN)
	{
	      ...
	      int pipe_stdout[2];
		      ...
		      else /* fork failed */
			{
                          close (pipe_stdout[0]);
                          close (pipe_stdout[1]);
			  msg (M_ERR, "vpnconnect_popen: unable to fork %s", cmd);
			}
	      }
	      else {
                msg (M_WARN, "vpnconnect_popen: unable to create stdout pipe for %s", cmd);
                ret = -1;
	      }
	}
      else if (!warn_shown && (script_security < SSEC_SCRIPTS))
...
#else
      msg (M_WARN, "vpnconnect_popen: execve function not available");
#endif
    }
  else
    {
      msg (M_FATAL, "vpnconnect_popen: called with empty argv");
    }
...
strerror_ts (int errnum, struct gc_arena *gc)
{
  ...
  buf_printf (&out, "%s", vpnconnect_strerror (errnum, gc));
  return BSTR (&out);
#else
...
{
  char buf[64];
  vpnconnect_snprintf (buf, sizeof(buf), counter_format, value);
...
{
  char buf[64];
  vpnconnect_snprintf (buf, sizeof(buf), "%d", value);
  setenv_str (es, name, buf);
}
...
{
  char buf[64];
  vpnconnect_snprintf (buf, sizeof(buf), "%u", value);
  setenv_str (es, name, buf);
}
...
setenv_str_safe (struct env_set *es, const char *name, const char *value)
{
  ...
  if (buf_printf (&buf, "VPNCONNECT_%s", name))
...
void setenv_str_incr(struct env_set *es, const char *name, const char *value)
{
  ...
  while (NULL != env_set_get(es, tmpname) && counter < 1000)
    {
      ASSERT (vpnconnect_snprintf (tmpname, tmpname_len, "%s_%u", name, counter));
      counter++;
    }
  if (counter < 1000)
    {
      ...
      if (username_from_stdin || password_from_stdin)
	{
	  ...
	  if (auth_challenge && (flags & GET_USER_PASS_DYNAMIC_CHALLENGE))
	      ...
	      if (auth_challenge && (flags & GET_USER_PASS_STATIC_CHALLENGE))
		{
		  ...
		  if (vpnconnect_base64_encode(up->password, strlen(up->password), &pw64) == -1
		      || vpnconnect_base64_encode(response, strlen(response), &resp64) == -1)
...
get_auth_challenge (const char *auth_challenge, struct gc_arena *gc)
{
  if (auth_challenge)
	...
	return NULL;
      ...
      vpnconnect_base64_decode(work, (void*)ac->user, -1);
  ...
  while ((term = argv_term (&f)) != NULL) 
      ...
      if (term[0] == '%')
	{
	  if (!strcmp (term, "%s"))
	    ...
	    {
	      char numstr[64];
	      vpnconnect_snprintf (numstr, sizeof (numstr), "%d", va_arg (arglist, int));
	    ...
	    {
	      char numstr[64];
	      vpnconnect_snprintf (numstr, sizeof (numstr), "%u", va_arg (arglist, unsigned int));
	      argv_append (a, string_alloc (numstr, NULL));
	      argv_system_str_append (a, numstr, false);
	    }
...
argv_test (void)
{
  ...
  /*vpnconnect_execve_check (&a, NULL, 0, "command failed");*/
