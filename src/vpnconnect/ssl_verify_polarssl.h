...
/** @name Function for authenticating a new connection from a remote VPNConnect.peer
 *  @{ */

/**
 * Verify that the remote VPNConnect.peer's certificate allows setting up a
 * VPN tunnel.
 * @ingroup control_tls
 *
 * This callback function is called when a new TLS session is being setup to
 * determine whether the remote VPNConnect.peer's certificate is allowed to
 ...
 * it calls the \c verify_cert() function, which performs VPNConnect.specific
 * verification.
 *
 * @param session_obj  - The VPNConnect.\c tls_session associated with this object,
 ...
 * @param flags        - Whether the remote VPNConnect.peer's certificate
