  ...
  if (strcmp (line, "unknown"))
      ...
      if (div)
	{
	  *div++ = '\0';
      ...
      if (vpnconnect_inet_aton (line, &network) != OIA_IP)
	{
...
pf_init_context (struct context *c)
{
  struct gc_arena gc = gc_new ();
#ifdef PLUGIN_PF
  if (plugin_defined (c->plugins, VPNCONNECT_PLUGIN_ENABLE_PF))
    {
      const char *pf_file = create_temp_file (c->options.tmp_dir, "pf", &gc);
      if( pf_file ) {
        setenv_str (c->c2.es, "pf_file", pf_file);

        if (plugin_call (c->plugins, VPNCONNECT_PLUGIN_ENABLE_PF, NULL, NULL, c->c2.es) == VPNCONNECT_PLUGIN_FUNC_SUCCESS)
          {
          ...
          {
            msg (M_WARN, "WARNING: VPNCONNECT_PLUGIN_ENABLE_PF disabled");
          }
      }
    }
#endif
