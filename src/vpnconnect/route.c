  ...
  if(get_special_addr (rl, ro->network, (in_addr_t *) &special.s_addr, &status))
    {
      ...
      ret = vpnconnect_getaddrinfo(0, inet_ntoa(special), NULL, 0, NULL,
                                AF_INET, network_list);
    }
  else
    ret = vpnconnect_getaddrinfo(GETADDR_RESOLVE | GETADDR_WARN_ON_SIGNAL,
	   ...
	   const struct env_set *es)
{
  ...
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: Linux route add command failed");
...
#elif defined (WIN32)
  {
	...
	status = vpnconnect_execve_check (&argv, es, 0, "ERROR: Windows route add command failed");
	...
	if (!status)
	  {
	    ...
	    netcmd_semaphore_lock ();
	    status = vpnconnect_execve_check (&argv, es, 0, "ERROR: Windows route add command failed [adaptive]");
	    netcmd_semaphore_release ();
	  }
      }
    else
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: Solaris route add command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: FreeBSD route add command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: DragonFly route add command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: OS X route add command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: OpenBSD/NetBSD route add command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: AIX route add command failed");
  }
...
add_route_ipv6 (struct route_ipv6 *r6, const struct tuntap *tt, unsigned int flags, const struct env_set *es)
{
...
#endif  /*ENABLE_IPROUTE*/
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: Linux route -6/-A inet6 add command failed");
  ...
  if (tt->options.msg_channel)
	...
	argv_printf_cat (&argv, " METRIC %d", r->metric);
      ...
      netcmd_semaphore_lock ();
      status = vpnconnect_execve_check (&argv, es, 0, "ERROR: Windows route add ipv6 command failed");
      netcmd_semaphore_release ();
    }
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: Solaris route add -inet6 command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: *BSD route add -inet6 command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: MacOS X route add -inet6 command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: OpenBSD route add -inet6 command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  status = vpnconnect_execve_check (&argv, es, 0, "ERROR: NetBSD route add -inet6 command failed");
	      ...
	      const struct env_set *es)
{
  ...
  if (r->flags & RT_METRIC_DEFINED)
    argv_printf_cat (&argv, "metric %d", r->metric);
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: Linux route delete command failed");
  ...
  if ((flags & ROUTE_METHOD_MASK) == ROUTE_METHOD_SERVICE)
    ...
    {
      netcmd_semaphore_lock ();
      vpnconnect_execve_check (&argv, es, 0, "ERROR: Windows route delete command failed");
      netcmd_semaphore_release ();
    }
  else if ((flags & ROUTE_METHOD_MASK) == ROUTE_METHOD_ADAPTIVE)
    {
      ...
      if (!status)
	{
	  ...
	  netcmd_semaphore_lock ();
	  vpnconnect_execve_check (&argv, es, 0, "ERROR: Windows route delete command failed [adaptive]");
	  netcmd_semaphore_release ();
	}
    }
  else
  ...
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: Solaris route delete command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: FreeBSD route delete command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: DragonFly route delete command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: OS X route delete command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: OpenBSD/NetBSD route delete command failed");
  ...
  argv_printf (&argv, "%s delete -net %s/%d %s",
		ROUTE_PATH,
	        network, netbits, gateway);
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: AIX route delete command failed");
  }
...
delete_route_ipv6 (const struct route_ipv6 *r6, const struct tuntap *tt, unsigned int flags, const struct env_set *es)
{
...
#endif  /*ENABLE_IPROUTE*/
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: Linux route -6/-A inet6 del command failed");
  ...
  if (tt->options.msg_channel)
	...
	argv_printf_cat (&argv, "METRIC %d", r->metric);
      ...
      netcmd_semaphore_lock ();
      vpnconnect_execve_check (&argv, es, 0, "ERROR: Windows route delete ipv6 command failed");
      netcmd_semaphore_release ();
    }
  ...
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: Solaris route delete -inet6 command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: *BSD route delete -inet6 command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: MacOS X route delete -inet6 command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: OpenBSD route delete -inet6 command failed");
  ...
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: NetBSD route delete -inet6 command failed");
  ...
  argv_printf (&argv, "%s delete -inet6 %s/%d %s",
		ROUTE_PATH,
	        network, r6->netbits, gateway);
  argv_msg (D_ROUTE, &argv);
  vpnconnect_execve_check (&argv, es, 0, "ERROR: AIX route add command failed");
...
get_default_gateway (struct route_gateway_info *rgi)
{
   ...
   * and serves almost no purpose since VPNConnect.only uses the
 ...
 * the data members are optional, however certain VPNConnect.functionality
