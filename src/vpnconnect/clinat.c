...
#if 0
static void
print_checksum (struct vpnconnect_iphdr *iph, const char *prefix)
{
  uint16_t *sptr;
  unsigned int sum = 0;
  int i = 0;
  for (sptr = (uint16_t *)iph; (uint8_t *)sptr < (uint8_t *)iph + sizeof(struct vpnconnect_iphdr); sptr++)
    {
      i += 1;
      sum += *sptr;
    }
...
static void
print_pkt (struct vpnconnect_iphdr *iph, const char *prefix, const int direction, const int msglevel)
{
  struct gc_arena gc = gc_new ();
		      ...
		      const int direction)
{
  ...
  if (alog)
	...
	print_pkt (&h->ip, "AFTER", direction, D_CLIENT_NAT);
      ...
      if (h->ip.protocol == VPNCONNECT_IPPROTO_TCP)
	{
	  if (BLEN(ipbuf) >= sizeof(struct vpnconnect_iphdr) + sizeof(struct vpnconnect_tcphdr))
	    {
	      ADJUST_CHECKSUM(accumulate, h->u.tcp.check);
	    }
	}
      else if (h->ip.protocol == VPNCONNECT_IPPROTO_UDP)
	{
	  if (BLEN(ipbuf) >= sizeof(struct vpnconnect_iphdr) + sizeof(struct vpnconnect_udphdr))
	    {
