...
man_help ()
{
  ...
  msg (M_CLIENT, "password type p        : Enter password p for a queried VPNConnect.password.");
  ...
  msg (M_CLIENT, "pid                    : Show process ID of the current VPNConnect.process.");
#ifdef ENABLE_PKCS11
  ...
  msg (M_CLIENT, "username type u        : Enter username u for a queried VPNConnect.username.");
...
man_state_name (const int state)
{
  switch (state)
    {
    case VPNCONNECT_STATE_INITIAL:
      return "INITIAL";
    case VPNCONNECT_STATE_CONNECTING:
      return "CONNECTING";
    case VPNCONNECT_STATE_WAIT:
      return "WAIT";
    case VPNCONNECT_STATE_AUTH:
      return "AUTH";
    case VPNCONNECT_STATE_GET_CONFIG:
      return "GET_CONFIG";
    case VPNCONNECT_STATE_ASSIGN_IP:
      return "ASSIGN_IP";
    case VPNCONNECT_STATE_ADD_ROUTES:
      return "ADD_ROUTES";
    case VPNCONNECT_STATE_CONNECTED:
      return "CONNECTED";
    case VPNCONNECT_STATE_RECONNECTING:
      return "RECONNECTING";
    case VPNCONNECT_STATE_EXITING:
      return "EXITING";
    case VPNCONNECT_STATE_RESOLVE:
      return "RESOLVE";
    case VPNCONNECT_STATE_TCP_CONNECT:
      return "TCP_CONNECT";
    default:
      return "?";
    }
...
{
  msg (M_CLIENT, ">INFO:VPNConnect.Management Interface Version %d -- type 'help' for more info",
       MANAGEMENT_VERSION);
...
man_bytecount_output_client (struct management *man)
{
  ...
  vpnconnect_snprintf (in, sizeof (in), counter_format, man->persist.bytes_in);
  vpnconnect_snprintf (out, sizeof (out), counter_format, man->persist.bytes_out);
			     ...
			     struct man_def_auth_context *mdac)
{
  ...
  char out[32];
  /* do in a roundabout way to work around possible mingw or mingw-glibc bug */
  vpnconnect_snprintf (in, sizeof (in), counter_format, *bytes_in_total);
  vpnconnect_snprintf (out, sizeof (out), counter_format, *bytes_out_total);
...
{
  /* Called to signal the VPNConnect.that the network configuration has changed and
     the client should either float or reconnect.

     The code is currently only used by ics-vpnconnect
  */
...
man_dispatch_command (struct management *man, struct status_output *so, const char **p, const int nparms)
{
  ...
  if (streq (p[0], "exit") || streq (p[0], "quit"))
    ...
    {
      msg (M_CLIENT, "VPNConnect Version: %s", title_string);
...
man_connect (struct management *man)
{
  ...
  if (man->settings.flags & MF_UNIX_SOCK)
...
#endif
    {
      ...
      status = vpnconnect_connect (man->connection.sd_cli,
...
{
  const int err = vpnconnect_errno ();
...
/*
 * The android control method will instruct the GUI part of vpnconnect to do
	  ...
	  if (streq (addr, "tunnel") && !(flags & MF_CONNECT_AS_CLIENT))
	  ...
	  else
	    {
		  ...
		  resolve_flags |= GETADDR_PASSIVE;
              ...
              status = vpnconnect_getaddrinfo (resolve_flags, addr, port, 0,
...
management_set_state (struct management *man,
		      ...
		      const char *detail,
                      ...
                      const struct vpnconnect_sockaddr *local,
                      const struct vpnconnect_sockaddr *remote)
{
  if (man->persist.state && (!(man->settings.flags & MF_SERVER) || state < VPNCONNECT_STATE_CLIENT_BASE))
    {
      struct gc_arena gc = gc_new ();
      struct log_entry e;
      const char *out = NULL;
...
management_post_tunnel_open (struct management *man, const in_addr_t tun_local_ip)
{
  ...
  if (man->settings.management_over_tunnel
      && man->connection.state == MS_INITIAL)
    {
      ...
      ret = vpnconnect_getaddrinfo(GETADDR_PASSIVE, inet_ntoa(ia), NULL, 0, NULL,
...
/*
 * Return true if (from the management interface's perspective) VPNConnect.should
 * daemonize.
 */
bool
