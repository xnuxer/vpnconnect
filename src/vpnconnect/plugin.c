...
plugin_type_name (const int type)
{
  switch (type)
    ...
    case VPNCONNECT_PLUGIN_CLIENT_CONNECT:
    ...
    case VPNCONNECT_PLUGIN_ENABLE_PF:
      return "PLUGIN_ENABLE_PF";
    case VPNCONNECT_PLUGIN_ROUTE_PREDOWN:
...
plugin_mask_string (const unsigned int type_mask, struct gc_arena *gc)
{
  ...
  for (i = 0; i < VPNCONNECT_PLUGIN_N; ++i)
    {
      if (VPNCONNECT_PLUGIN_MASK (i) & type_mask)
	{
	  if (!first)
	    buf_printf (&out, "|");
...
{
  return ((1<<VPNCONNECT_PLUGIN_N)-1);
}
  ...
  if (!absolute_pathname (p->so_pathname))
    {
      ...
      vpnconnect_snprintf (full, sizeof(full), "%s/%s", PLUGIN_LIBDIR, p->so_pathname);
...
# define PLUGIN_SYM(var, name, flags) dll_resolve_symbol (p->module, (void*)&p->var, name, p->so_pathname, flags)
  ...
  PLUGIN_SYM (open1, "vpnconnect_plugin_open_v1", 0);
  PLUGIN_SYM (open2, "vpnconnect_plugin_open_v2", 0);
  PLUGIN_SYM (open3, "vpnconnect_plugin_open_v3", 0);
  PLUGIN_SYM (func1, "vpnconnect_plugin_func_v1", 0);
  PLUGIN_SYM (func2, "vpnconnect_plugin_func_v2", 0);
  PLUGIN_SYM (func3, "vpnconnect_plugin_func_v3", 0);
  PLUGIN_SYM (close, "vpnconnect_plugin_close_v1", PLUGIN_SYMBOL_REQUIRED);
  PLUGIN_SYM (abort, "vpnconnect_plugin_abort_v1", 0);
  PLUGIN_SYM (client_constructor, "vpnconnect_plugin_client_constructor_v1", 0);
  PLUGIN_SYM (client_destructor, "vpnconnect_plugin_client_destructor_v1", 0);
  PLUGIN_SYM (min_version_required, "vpnconnect_plugin_min_version_required_v1", 0);
  PLUGIN_SYM (initialization_point, "vpnconnect_plugin_select_initialization_point_v1", 0);

  if (!p->open1 && !p->open2 && !p->open3)
    msg (M_FATAL, "PLUGIN: symbol vpnconnect_plugin_open_vX is undefined in plugin: %s", p->so_pathname);

  if (!p->func1 && !p->func2 && !p->func3)
    msg (M_FATAL, "PLUGIN: symbol vpnconnect_plugin_func_vX is undefined in plugin: %s", p->so_pathname);
  ...
  if (p->min_version_required)
    {
      ...
      if (plugin_needs_version > VPNCONNECT_PLUGIN_VERSION)
	msg (M_FATAL, "PLUGIN_INIT: plugin needs interface version %d, but this version of VPNConnect.only supports version %d: %s",
	     plugin_needs_version,
	     VPNCONNECT_PLUGIN_VERSION,
	     p->so_pathname);
    }
  ...
  if (p->initialization_point)
    ...
    p->requested_initialization_point = VPNCONNECT_PLUGIN_INIT_PRE_DAEMON;
...
static void
plugin_log (vpnconnect_plugin_log_flags_t flags, const char *name, const char *format, ...)
{
  va_list arglist;
...
static struct vpnconnect_plugin_callbacks callbacks = {
  plugin_log,
  plugin_vlog
};
...
# define _VPNCONNECT_PATCH_LEVEL VPNCONNECT_VERSION_PATCH
#else
# define _VPNCONNECT_PATCH_LEVEL "git:" CONFIGURE_GIT_REVISION CONFIGURE_GIT_FLAGS
#endif
...
{
  int status = VPNCONNECT_PLUGIN_FUNC_SUCCESS;

  /* clear return list */
  if (retlist)
    *retlist = NULL;

  if (p->plugin_handle && (p->plugin_type_mask & VPNCONNECT_PLUGIN_MASK (type)))
    {
      struct gc_arena gc = gc_new ();
       ...
       */
      if (p->func3) {
        struct vpnconnect_plugin_args_func_in args = { type,
        ...
        struct vpnconnect_plugin_args_func_return retargs;

        CLEAR(retargs);
        retargs.return_list = retlist;
        status = (*p->func3)(VPNCONNECT_PLUGINv3_STRUCTVER, &args, &retargs);
      } else if (p->func2)
	...
	ASSERT (0);
      ...
      if (status == VPNCONNECT_PLUGIN_FUNC_ERROR)
	    ...
	    )
{
  ...
  if (plugin_defined (pl, type))
      ...
      for (i = 0; i < n; ++i)
	{
	  ...
	  switch (status)
	    {
	    case VPNCONNECT_PLUGIN_FUNC_SUCCESS:
	      success = true;
	      break;
	    case VPNCONNECT_PLUGIN_FUNC_DEFERRED:
	      deferred = true;
	      break;
	    default:
	      error = true;
	...
	pr->n = i;
      ...
      if (type == VPNCONNECT_PLUGIN_ENABLE_PF && success)
	return VPNCONNECT_PLUGIN_FUNC_SUCCESS;
      else if (error)
	return VPNCONNECT_PLUGIN_FUNC_ERROR;
      else if (deferred)
	return VPNCONNECT_PLUGIN_FUNC_DEFERRED;
    }
...
plugin_defined (const struct plugin_list *pl, const int type)
{
  ...
  if (pl)
	...
	{
	  int i;
	  const unsigned int mask = VPNCONNECT_PLUGIN_MASK (type);
	  for (i = 0; i < pc->n; ++i)
	    {
...
static void
vpnconnect_plugin_string_list_free (struct vpnconnect_plugin_string_list *l)
{
  struct vpnconnect_plugin_string_list *next;
  while (l)
    {
      next = l->next;
      vpnconnect_plugin_string_list_item_free (l);
      l = next;
    }
}

static struct vpnconnect_plugin_string_list *
vpnconnect_plugin_string_list_find (struct vpnconnect_plugin_string_list *l, const char *name)
{
  while (l)
    {
			  ...
			  const char *colname)
{
  ...
  dest->n = 0;
  for (i = 0; i < src->n; ++i)
    dest->list[i] = vpnconnect_plugin_string_list_find (src->list[i], colname);
  dest->n = i;
}
...
plugin_return_free (struct plugin_return *pr)
{
  int i;
  for (i = 0; i < pr->n; ++i)
    vpnconnect_plugin_string_list_free (pr->list[i]);
  pr->n = 0;
}
...
plugin_return_print (const int msglevel, const char *prefix, const struct plugin_return *pr)
{
  ...
  for (i = 0; i < pr->n; ++i)
    {
      struct vpnconnect_plugin_string_list *l = pr->list[i];
      int count = 0;
