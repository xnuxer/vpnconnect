                      ...
                      )
{
  ...
  NET_LUID tapluid;
  UINT64 filterid;
  FWP_BYTE_BLOB *vpnconnectblob = NULL;
  FWPM_FILTER0 Filter = {0};
  FWPM_FILTER_CONDITION0 Condition[2] = {0};
  WCHAR *FIREWALL_NAME = L"VPNConnect";
  DWORD err = 0;
  ...
  *engine_handle = NULL;
  ...
  err = FwpmGetAppIdFromFileName0 (exe_path, &vpnconnectblob);
  CHECK_ERROR (err, "Get byte blob for vpnconnect executable name failed");
  ...
  /* First filter. Permit IPv4 DNS queries from VPNConnect.itself. */
  ...
  Condition[1].conditionValue.byteBlob = vpnconnectblob;

  err = FwpmFilterAdd0(*engine_handle, &Filter, NULL, &filterid);
  CHECK_ERROR (err, "Add filter to permit IPv4 port 53 traffic from VPNConnect.failed");

  /* Second filter. Permit IPv6 DNS queries from VPNConnect.itself. */
  Filter.layerKey = FWPM_LAYER_ALE_AUTH_CONNECT_V6;

  err = FwpmFilterAdd0(*engine_handle, &Filter, NULL, &filterid);
  CHECK_ERROR (err, "Add filter to permit IPv6 port 53 traffic from VPNConnect.failed");
...
out:
  ...
  if (vpnconnectblob)
    FwpmFreeMemory0 ((void **)&vpnconnectblob);
