...
/*
 * Verify certificate for the given session. Performs VPNConnect.specific
 * verification.
 *
 ...
 */
result_t verify_cert(struct tls_session *session, vpnconnect_x509_cert_t *cert, int cert_depth);
 ...
 */
char *x509_get_subject (vpnconnect_x509_cert_t *cert, struct gc_arena *gc);
 ...
 */
unsigned char *x509_get_sha1_hash (vpnconnect_x509_cert_t *cert, struct gc_arena *gc);
...
result_t backend_x509_get_username (char *common_name, int cn_len,
    char * x509_username_field, vpnconnect_x509_cert_t *peer_cert);
 ...
 */
char *backend_x509_get_serial (vpnconnect_x509_cert_t *cert, struct gc_arena *gc);
 ...
 */
char *backend_x509_get_serial_hex (vpnconnect_x509_cert_t *cert,
    struct gc_arena *gc);
 ...
 */
void x509_setenv (struct env_set *es, int cert_depth, vpnconnect_x509_cert_t *cert);
...
void x509_setenv_track (const struct x509_track *xt, struct env_set *es,
    const int depth, vpnconnect_x509_cert_t *x509);
 ...
 */
result_t x509_verify_ns_cert_type(const vpnconnect_x509_cert_t *cert, const int usage);
 ...
 */
result_t x509_verify_cert_ku (vpnconnect_x509_cert_t *x509, const unsigned * const expected_ku,
    int expected_len);
 ...
 */
result_t x509_verify_cert_eku (vpnconnect_x509_cert_t *x509, const char * const expected_oid);
 ...
 *
 */
result_t x509_write_pem(FILE *peercert_file, vpnconnect_x509_cert_t *peercert);
...
result_t x509_verify_crl(const char *crl_file, const char *crl_inline,
                         vpnconnect_x509_cert_t *cert, const char *subject);
