 ...
 * Not used if VPNCONNECT_DEBUG_COMMAND_LINE is defined.
 */
#define VPNCONNECT_MSG_FP   stdout
#define VPNCONNECT_ERROR_FP stderr

/*
 * Exit status codes
 */

#define VPNCONNECT_EXIT_STATUS_GOOD                    0
#define VPNCONNECT_EXIT_STATUS_ERROR                   1
#define VPNCONNECT_EXIT_STATUS_USAGE                   1
#define VPNCONNECT_EXIT_STATUS_CANNOT_OPEN_DEBUG_FILE  1

/*
 * Special command line debugging mode.
 * If VPNCONNECT_DEBUG_COMMAND_LINE
 * is defined, contents of argc/argv will
 * be dumped to VPNCONNECT_DEBUG_FILE as well
 * as all other VPNConnect.messages.
 */

/* #define VPNCONNECT_DEBUG_COMMAND_LINE */
#define VPNCONNECT_DEBUG_FILE PACKAGE ".log"

/* String and Error functions */

#ifdef WIN32
# define vpnconnect_errno()             GetLastError()
# define vpnconnect_strerror(e, gc)     strerror_win32(e, gc)
  const char *strerror_win32 (DWORD errnum, struct gc_arena *gc);
#else
# define vpnconnect_errno()             errno
# define vpnconnect_strerror(x, gc)     strerror(x)
#endif
...
/* exit program */
void vpnconnect_exit (const int status);
