...
{
  const struct vpnconnect_iphdr *pip;
  int hlen;

  if (BLEN (buf) < (int) sizeof (struct vpnconnect_iphdr))
    return;
  
  verify_align_4 (buf);
  pip = (struct vpnconnect_iphdr *) BPTR (buf);

  hlen = VPNCONNECT_IPH_GET_LEN (pip->version_len);

  if (pip->protocol == VPNCONNECT_IPPROTO_TCP
      && ntohs (pip->tot_len) == BLEN (buf)
      && (ntohs (pip->frag_off) & VPNCONNECT_IP_OFFMASK) == 0
      && hlen <= BLEN (buf)
      && BLEN (buf) - hlen
         >= (int) sizeof (struct vpnconnect_tcphdr))
    {
      struct buffer newbuf = *buf;
      if (buf_advance (&newbuf, hlen))
	{
	  struct vpnconnect_tcphdr *tc = (struct vpnconnect_tcphdr *) BPTR (&newbuf);
	  if (tc->flags & VPNCONNECT_TCPH_SYN_MASK)
...
{
  const struct vpnconnect_ipv6hdr *pip6;
  struct buffer newbuf;

  if (BLEN (buf) < (int) sizeof (struct vpnconnect_ipv6hdr))
    return;

  verify_align_4 (buf);
  pip6 = (struct vpnconnect_ipv6hdr *) BPTR (buf);
   ...
   * fragment, mobility) are very unlikely to be seen inside a VPNConnect
   * tun, so for now, we only handle the case of "single next header = TCP"
   */
  if ( pip6->nexthdr != VPNCONNECT_IPPROTO_TCP )
    return;
...
mss_fixup_dowork (struct buffer *buf, uint16_t maxmss)
{
  ...
  uint16_t mssval;
  int accumulate;
  struct vpnconnect_tcphdr *tc;

  ASSERT (BLEN (buf) >= (int) sizeof (struct vpnconnect_tcphdr));

  verify_align_4 (buf);
  tc = (struct vpnconnect_tcphdr *) BPTR (buf);
  hlen = VPNCONNECT_TCPH_GET_DOFF (tc->doff_res);

  /* Invalid header length or header without options. */
  if (hlen <= (int) sizeof (struct vpnconnect_tcphdr)
      || hlen > BLEN (buf))
    return;

  for (olen = hlen - sizeof (struct vpnconnect_tcphdr),
	 opt = (uint8_t *)(tc + 1);
       olen > 0;
       olen -= optlen, opt += optlen) {
    if (*opt == VPNCONNECT_TCPOPT_EOL)
      break;
    else if (*opt == VPNCONNECT_TCPOPT_NOP)
      optlen = 1;
    else {
      optlen = *(opt + 1);
      if (optlen <= 0 || optlen > olen)
        break;
      if (*opt == VPNCONNECT_TCPOPT_MAXSEG) {
        if (optlen != VPNCONNECT_TCPOLEN_MAXSEG)
          continue;
