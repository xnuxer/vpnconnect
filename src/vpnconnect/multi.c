		      ...
		      const struct mroute_addr *addr)
{
  ...
  if (plugin_defined (plugins, VPNCONNECT_PLUGIN_LEARN_ADDRESS))
    {
	...
	argv_printf_cat (&argv, "%s", tls_common_name (mi->context.c2.tls_multi, false));
      if (plugin_call (plugins, VPNCONNECT_PLUGIN_LEARN_ADDRESS, &argv, NULL, es) != VPNCONNECT_PLUGIN_FUNC_SUCCESS)
	{
  ...
  if (m->top.options.learn_address_script)
    {
      ...
      if (mi)
	argv_printf_cat (&argv, "%s", tls_common_name (mi->context.c2.tls_multi, false));
      if (!vpnconnect_run_script (&argv, es, 0, "--learn-address"))
	ret = false;
      argv_reset (&argv);
    }
				...
				struct multi_instance *mi)
{
  if ((mi->context.c2.context_auth == CAS_SUCCEEDED && mi->connection_established_flag)
      || mi->context.c2.context_auth == CAS_PARTIAL)
    {
      ...
      if (plugin_defined (mi->context.plugins, VPNCONNECT_PLUGIN_CLIENT_DISCONNECT))
	{
	  if (plugin_call (mi->context.plugins, VPNCONNECT_PLUGIN_CLIENT_DISCONNECT, NULL, NULL, mi->context.c2.es) != VPNCONNECT_PLUGIN_FUNC_SUCCESS)
      ...
      if (mi->context.options.client_disconnect_script)
	{
	  ...
	  vpnconnect_run_script (&argv, mi->context.c2.es, 0, "--client-disconnect");
	  argv_reset (&argv);
	}
#ifdef MANAGEMENT_DEF_AUTH
      if (management)
...
multi_print_status (struct multi_context *m, struct status_output *so, const int version)
{
  if (m->hash)
      ...
      if (version == 1) /* WAS: m->status_file_version */
	{
	   ...
	   */
	  status_printf (so, "VPNConnect CLIENT LIST");
...
multi_learn_in_addr_t (struct multi_context *m,
		       ...
		       bool primary)
{
  struct vpnconnect_sockaddr remote_si;
  struct mroute_addr addr;

  CLEAR (remote_si);
  remote_si.addr.in4.sin_family = AF_INET;
  remote_si.addr.in4.sin_addr.s_addr = htonl (a);
  ASSERT (mroute_extract_vpnconnect_sockaddr (&addr, &remote_si, false));
...
multi_connection_established (struct multi_context *m, struct multi_instance *mi)
{
  if (tls_authentication_status (mi->context.c2.tls_multi, 0) == TLS_AUTHENTICATION_SUCCEEDED)
      ...
      if (mi->context.options.client_config_dir)
	{
      ...
      if (plugin_defined (mi->context.plugins, VPNCONNECT_PLUGIN_CLIENT_CONNECT))
	{
	  struct argv argv = argv_new ();
	  ...
	  if (plugin_call (mi->context.plugins, VPNCONNECT_PLUGIN_CLIENT_CONNECT, &argv, NULL, mi->context.c2.es) != VPNCONNECT_PLUGIN_FUNC_SUCCESS)
	    {
      ...
      if (plugin_defined (mi->context.plugins, VPNCONNECT_PLUGIN_CLIENT_CONNECT_V2))
	{
	  struct plugin_return pr;

	  plugin_return_init (&pr);

	  if (plugin_call (mi->context.plugins, VPNCONNECT_PLUGIN_CLIENT_CONNECT_V2, NULL, &pr, mi->context.c2.es) != VPNCONNECT_PLUGIN_FUNC_SUCCESS)
	    {
      ...
      if (mi->context.options.client_connect_script && cc_succeeded)
	{
	  ...
	  if (vpnconnect_run_script (&argv, mi->context.c2.es, 0, "--client-connect"))
	    {
	      multi_client_connect_post (m, mi, dc_file, option_permissions_mask, &option_types_found);
	      ++cc_succeeded_count;
	    }
...
multi_schedule_context_wakeup (struct multi_context *m, struct multi_instance *mi)
{
  ...
  ASSERT (!vpnconnect_gettimeofday (&mi->wakeup, NULL));
...
void multi_process_float (struct multi_context* m, struct multi_instance* mi)
{
  ...
  if (!mroute_extract_vpnconnect_sockaddr (&real, &m->top.c2.from.dest, true))
    goto done;
...
multi_push_restart_schedule_exit(struct multi_context *m, bool next_server)
{
  ...
  /* reschedule signal */
  ASSERT (!vpnconnect_gettimeofday (&m->deferred_shutdown_signal.wakeup, NULL));
  tv.tv_sec = 2;
  tv.tv_usec = 0;
...
management_callback_kill_by_addr (void *arg, const in_addr_t addr, const int port)
{
  ...
  struct hash_iterator hi;
  struct hash_element *he;
  struct vpnconnect_sockaddr saddr;
  ...
  if (mroute_extract_vpnconnect_sockaddr (&maddr, &saddr, true))
    {
      hash_iterator_init (m->iter, &hi);
      while ((he = hash_iterator_next (&hi)))
	{
