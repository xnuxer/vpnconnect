...
/*
 * A foreign (non-VPNConnect. connection we are proxying,
 * usually HTTPS
 */
...
journal_add (const char *journal_dir, struct proxy_connection *pc, struct proxy_connection *cp)
{
  ...
  struct vpnconnect_sockaddr from, to;
  socklen_t slen, dlen;
  int fnlen;
  char *jfn;
  int fd;
  ...
  if (!getpeername (pc->sd, (struct sockaddr *) &from.addr.sa, &slen)
    ...
    {
      const char *f = print_vpnconnect_sockaddr (&from, &gc);
      const char *t = print_vpnconnect_sockaddr (&to, &gc);
      fnlen =  strlen(journal_dir) + strlen(t) + 2;
      jfn = (char *) malloc(fnlen);
      check_malloc_return (jfn);
      vpnconnect_snprintf (jfn, fnlen, "%s/%s", journal_dir, t);
		 ...
		 const char *journal_dir)
{
  ...
  status = vpnconnect_connect (sd_server,(const struct sockaddr*)  &server_addr, 5, NULL);
  if (status)
    {
      msg (M_WARN, "PORT SHARE PROXY: connect to port-share server failed");
      vpnconnect_close_socket (sd_server);
      return false;
    }
...
/*
 * Called from the main VPNConnect.process to enable the port
 * share proxy.
 */
struct port_share *
		 ...
		 const char *journal_dir)
{
  ...
  status = vpnconnect_getaddrinfo (GETADDR_RESOLVE|GETADDR_FATAL,
...
port_share_close (struct port_share *ps)
{
  if (ps)
      ...
      if (ps->foreground_fd >= 0)
	{
	  ...
	  vpnconnect_close_socket (ps->foreground_fd);
	  ps->foreground_fd = -1;
	}
 ...
 * data payload, return true if the protocol is that of a VPNConnect
 * client attempting to connect with an VPNConnect.server.
 */
bool
is_vpnconnect_protocol (const struct buffer *buf)
{
