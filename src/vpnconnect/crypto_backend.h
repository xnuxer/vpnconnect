...
/* TLS uses a tag of 128 bytes, let's do the same for VPNConnect.*/
#define VPNCONNECT_AEAD_TAG_LENGTH 16

/** Struct used in cipher name translation table */
typedef struct {
  const char *vpnconnect_name;	/**< Cipher name used by VPNConnect.*/
 ...
 * used by both crypto and ssl components of VPNConnect.
 */
...
/**
 * Translate a data channel cipher name from the VPNConnect.config file
 * 'language' to the crypto library specific name.
 */
const char * translate_cipher_name_from_vpnconnect (const char *cipher_name);

/**
 * Translate a data channel cipher name from the crypto library specific name
 * to the VPNConnect.config file 'language'.
 */
const char * translate_cipher_name_from_vpnconnect (const char *cipher_name);
 ...
 *
 * @return 		Cipher mode, either \c VPNCONNECT_MODE_CBC, \c
 * 			VPNCONNECT_MODE_OFB or \c VPNCONNECT_MODE_CFB
 */
...
/**
 * Translate an VPNConnect.cipher name to a crypto library cipher name.
 *
 * @param cipher_name	An VPNConnect.cipher name
 *
...
/**
 * Translate a crypto library cipher name to an VPNConnect.cipher name.
 *
 * @param cipher_name	A crypto library cipher name
 *
 * @return		The corresponding VPNConnect.cipher name, or NULL if no
 * 			matching cipher name was found.
 */
const char * translate_cipher_name_to_vpnconnect (const char *cipher_name);
