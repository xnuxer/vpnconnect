...
make_base64_string2 (const uint8_t *str, int src_len, struct gc_arena *gc)
{
  uint8_t *ret = NULL;
  char *b64out = NULL;
  ASSERT (vpnconnect_base64_encode ((const void *)str, src_len, &b64out) >= 0);
...
add_proxy_headers (struct http_proxy_info *p,
		  ...
		  const char *host,	  /* vpnconnect server remote */
		  const char* port	  /* vpnconnect server port */
		  )
{
  char buf[512];
  int i;
  ...
  for  (i=0; i < MAX_CUSTOM_HTTP_HEADER && p->options.custom_headers[i].name;i++)
	...
	{
	  vpnconnect_snprintf (buf, sizeof(buf), "%s: %s",
	...
	{
	  vpnconnect_snprintf (buf, sizeof(buf), "%s",
			    p->options.custom_headers[i].name);
  ...
  if (!host_header_sent)
    {
      vpnconnect_snprintf (buf, sizeof(buf), "Host: %s", host);
      msg (D_PROXY, "Send to HTTP proxy: '%s'", buf);
      if (!send_line_crlf(sd, buf))
        return false;
    }
  ...
  if (p->options.user_agent)
    {
      vpnconnect_snprintf (buf, sizeof(buf), "User-Agent: %s",
			       ...
			       volatile int *signal_received)
{
  ...
  if (p->auth_method == HTTP_AUTH_DIGEST && p->proxy_authenticate)
  ...
  else
    {
      ...
      vpnconnect_snprintf (buf, sizeof(buf), "CONNECT %s:%s HTTP/%s",
			host,
			port,
	...
	case HTTP_AUTH_BASIC:
	  vpnconnect_snprintf (buf, sizeof(buf), "Proxy-Authorization: Basic %s",
	...
	case HTTP_AUTH_NTLM2:
	  ...
	  vpnconnect_snprintf (buf, sizeof(buf), "Proxy-Connection: Keep-Alive");
	  if (!send_line_crlf (sd, buf))
	    goto error;

	  vpnconnect_snprintf (buf, sizeof(buf), "Proxy-Authorization: NTLM %s",
			    ntlm_phase_1 (p, &gc));
  ...
  while (nparms >= 1 && status == 407)
      ...
      if (p->auth_method == HTTP_AUTH_BASIC && !processed)
	{
          ...
          while (true)
            {
              ...
              vpnconnect_snprintf (get, sizeof get, "%%*s NTLM %%%ds", (int) sizeof (buf2) - 1);
	  ...
	  msg (D_PROXY, "Attempting NTLM Proxy-Authorization phase 3");
	  {
	    ...
	    vpnconnect_snprintf (buf, sizeof(buf), "Proxy-Authorization: NTLM %s", np3);
	  }
	  ...
	  if (method == HTTP_AUTH_DIGEST)
	    {
	      ...
	      vpnconnect_snprintf (uri, sizeof(uri), "%s:%s",
				host,
				port);
	      ...
	      if (opaque)
		{
		  ...
		  vpnconnect_snprintf (opaque_kv, len, ", opaque=\"%s\"", opaque);
		}
	      ...
	      /* format HTTP CONNECT message */
	      vpnconnect_snprintf (buf, sizeof(buf), "%s %s HTTP/%s",
				http_method,
				uri,
				p->options.http_version);
		...
		goto error;
	      ...
	      vpnconnect_snprintf (buf, sizeof(buf), "Proxy-Authorization: Digest username=\"%s\", realm=\"%s\", nonce=\"%s\", uri=\"%s\", qop=%s, nc=%s, cnonce=\"%s\", response=\"%s\"%s",
				username,
				realm,
				nonce,
				uri,
   ...
   * start of the VPNConnect.data stream (put it in lookahead).
   */
