...
#ifdef VPNCONNECT_DEBUG_COMMAND_LINE
  {
    int i;
    for (i = 0; i < argc; ++i)
...
do_uid_gid_chroot (struct context *c, bool no_delay)
{
  ...
  if (c0 && !c0->uid_gid_chroot_set)
	...
	mstats_open(c->options.memstats_fn);
      ...
      /* Apply a SELinux context in order to restrict what VPNConnect.can do
 ...
 * parts of VPNConnect.might want to fill the route-list with info, e.g. DHCP)
 */
static void
		    ...
		    struct env_set *es)
{
  ...
  if (!init_route_list (route_list,
    ...
    {
      if (fatal)
	vpnconnect_exit (VPNCONNECT_EXIT_STATUS_ERROR);	/* exit point */
    }
  else
    {
  ...
  if (flags & ISC_ERRORS)
    {
      ...
      msg (M_INFO, "%s With Errors (dhcpclientserv)", message);
#else
  ...
  if (management)
    {
      ...
      struct vpnconnect_sockaddr local, remote;
	  ...
	  struct env_set *es)
{
  ...
  if (plugin_defined (plugins, VPNCONNECT_PLUGIN_ROUTE_UP))
    {
      if (plugin_call (plugins, VPNCONNECT_PLUGIN_ROUTE_UP, NULL, NULL, es) != VPNCONNECT_PLUGIN_FUNC_SUCCESS)
  ...
  if (options->route_script)
    {
      ...
      vpnconnect_run_script (&argv, es, 0, "--route-up");
      argv_reset (&argv);
    }
	  ...
	  if (c->c1.route_list || c->c1.route_ipv6_list )
           {
              run_up_down (c->options.route_predown_script,
                           c->plugins,
                           VPNCONNECT_PLUGIN_ROUTE_PREDOWN,
                           tuntap_actual,
#ifdef WIN32
                           adapter_index,
#endif
  ...
  if (!key_ctx_bi_defined (&c->c1.ks.static_key))
	...
	read_key_file (&key2, rkf_file, rkf_flags);
      ...
      init_key_ctx (&c->c1.ks.static_key.encrypt, &key2.keys[kds.out_key],
		    &c->c1.ks.key_type, VPNCONNECT_OP_ENCRYPT, "Static Encrypt");
      init_key_ctx (&c->c1.ks.static_key.decrypt, &key2.keys[kds.in_key],
		    &c->c1.ks.key_type, VPNCONNECT_OP_DECRYPT, "Static Decrypt");
...
/*
 * Initialize the persistent component of VPNConnect.s TLS mode,
...
do_init_crypto_tls (struct context *c, const unsigned int flags)
{
  ...
  /*
   * Initialize VPNConnect.s master TLS-mode object.
   */
...
do_option_warnings (struct context *c)
{
  ...
  if (o->tls_client
      ...
      && !o->remote_cert_eku)
    msg (M_WARN, "WARNING: No server certificate verification method has been enabled.");
#endif
...
init_instance (struct context *c, const struct env_set *env, const unsigned int flags)
{
  ...
  /* initialize plugins */
  if (c->mode == CM_P2P || c->mode == CM_TOP)
    open_plugins (c, false, VPNCONNECT_PLUGIN_INIT_PRE_DAEMON);
#endif
  ...
  /* initialize plugins */
  if (c->mode == CM_P2P || c->mode == CM_TOP)
    open_plugins (c, false, VPNCONNECT_PLUGIN_INIT_POST_DAEMON);
#endif
  ...
  /* initialize plugins */
  if (c->mode == CM_P2P || c->mode == CM_TOP)
    open_plugins (c, false, VPNCONNECT_PLUGIN_INIT_POST_UID_CHANGE);
#endif

#if PORT_SHARE
  /* share VPNConnect.port with foreign (such as HTTPS) server */
