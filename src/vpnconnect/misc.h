...
int vpnconnect_popen (const struct argv *a,  const struct env_set *es);
int vpnconnect_execve (const struct argv *a, const struct env_set *es, const unsigned int flags);
bool vpnconnect_execve_check (const struct argv *a, const struct env_set *es, const unsigned int flags, const char *error_message);
bool vpnconnect_execve_allowed (const unsigned int flags);

static inline bool
vpnconnect_run_script (const struct argv *a, const struct env_set *es, const unsigned int flags, const char *hook)
{
  char msg[256];

  vpnconnect_snprintf(msg, sizeof(msg), "WARNING: Failed running command (%s)", hook);
  return vpnconnect_execve_check(a, es, flags | S_SCRIPT, msg);
}
 ...
 */
void vpnconnect_sleep (const int n);
