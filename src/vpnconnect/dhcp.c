	      ...
	      const uint8_t *dest_addr)
{
  ...
  sum += (uint16_t) VPNCONNECT_IPPROTO_UDP + (uint16_t) len_udp;
...
dhcp_extract_router_msg (struct buffer *ipbuf)
{
  ...
  const int optlen = BLEN (ipbuf) - (sizeof (struct vpnconnect_iphdr) + sizeof (struct vpnconnect_udphdr) + sizeof (struct dhcp));

  if (optlen >= 0
      && df->ip.protocol == VPNCONNECT_IPPROTO_UDP
      ...
      if (message_type == DHCPACK || message_type == DHCPOFFER)
	{
	  ...
	  df->udp.check = htons (udp_checksum ((uint8_t *) &df->udp, 
					       sizeof (struct vpnconnect_udphdr) + sizeof (struct dhcp) + optlen,
					       (uint8_t *)&df->ip.saddr,
					       (uint8_t *)&df->ip.daddr));
