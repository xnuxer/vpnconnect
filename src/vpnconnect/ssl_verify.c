 ...
 */
static result_t
verify_peer_cert(const struct tls_options *opt, vpnconnect_x509_cert_t *peer_cert,
 ...
 */
static void
verify_cert_set_env(struct env_set *es, vpnconnect_x509_cert_t *peer_cert, int cert_depth,
    const char *subject, const char *common_name
    ...
    )
{
  ...
  vpnconnect_snprintf (envname, sizeof(envname), "tls_id_%d", cert_depth);
  setenv_str (es, envname, subject);

#if 0
  /* export common name string as environmental variable */
  vpnconnect_snprintf (envname, sizeof(envname), "tls_common_name_%d", cert_depth);
  ...
  /* export X509 cert SHA1 fingerprint */
  {
    ...
    vpnconnect_snprintf (envname, sizeof(envname), "tls_digest_%d", cert_depth);
  ...
  vpnconnect_snprintf (envname, sizeof(envname), "tls_serial_%d", cert_depth);
  ...
  vpnconnect_snprintf (envname, sizeof(envname), "tls_serial_hex_%d", cert_depth);
  setenv_str (es, envname, serial);
...
verify_cert_call_plugin(const struct plugin_list *plugins, struct env_set *es,
    int cert_depth, vpnconnect_x509_cert_t *cert, char *subject)
{
  if (plugin_defined (plugins, VPNCONNECT_PLUGIN_TLS_VERIFY))
    {
      int ret;
      ...
      ret = plugin_call_ssl (plugins, VPNCONNECT_PLUGIN_TLS_VERIFY, &argv, NULL, es, cert_depth, cert);

      argv_reset (&argv);

      if (ret == VPNCONNECT_PLUGIN_FUNC_SUCCESS)
	{
...
static const char *
verify_cert_export_cert(vpnconnect_x509_cert_t *peercert, const char *tmp_dir, struct gc_arena *gc)
{
  FILE *peercert_file;
...
verify_cert_call_command(const char *verify_command, struct env_set *es,
    int cert_depth, vpnconnect_x509_cert_t *cert, char *subject, const char *verify_export_cert)
{
  ...
  ret = vpnconnect_run_script (&argv, es, 0, "--tls-verify script");
 ...
 */
static result_t
verify_check_crl_dir(const char *crl_dir, vpnconnect_x509_cert_t *cert)
{
  ...
  if (!vpnconnect_snprintf(fn, sizeof(fn), "%s%c%s", crl_dir, OS_SPECIFIC_DIRSEP, serial))
    {
...
result_t
verify_cert(struct tls_session *session, vpnconnect_x509_cert_t *cert, int cert_depth)
{
  result_t ret = FAILURE;
  char *subject = NULL;
...
verify_user_pass_script (struct tls_session *session, const struct user_pass *up)
{
  ...
  if ((session->opt->ssl_flags & SSLF_AUTH_USER_PASS_OPTIONAL) || strlen (up->username))
      ...
      else
	{
      ...
      /* call command */
      ret = vpnconnect_run_script (&argv, session->opt->es, 0,
...
{
  int retval = VPNCONNECT_PLUGIN_FUNC_ERROR;
#ifdef PLUGIN_DEF_AUTH
  ...
  if ((session->opt->ssl_flags & SSLF_AUTH_USER_PASS_OPTIONAL) || strlen (up->username))
    {
      ...
      /* call command */
      retval = plugin_call (session->opt->plugins, VPNCONNECT_PLUGIN_AUTH_USER_PASS_VERIFY, NULL, NULL, session->opt->es);

#ifdef PLUGIN_DEF_AUTH
      /* purge auth control filename (and file itself) for non-deferred returns */
      if (retval != VPNCONNECT_PLUGIN_FUNC_DEFERRED)
	key_state_rm_auth_control_file (ks);
#endif
...
{
  int s1 = VPNCONNECT_PLUGIN_FUNC_SUCCESS;
  bool s2 = true;
  struct key_state *ks = &session->key[KS_PRIMARY]; 	   /* primary key */
...
#endif
  if (plugin_defined (session->opt->plugins, VPNCONNECT_PLUGIN_AUTH_USER_PASS_VERIFY))
  ...
  if ((session->opt->ssl_flags & SSLF_USERNAME_AS_COMMON_NAME) && strlen (up->username) > TLS_USERNAME_LEN)
    {
      ...
      s1 = VPNCONNECT_PLUGIN_FUNC_ERROR;
    }

  /* auth succeeded? */
  if ((s1 == VPNCONNECT_PLUGIN_FUNC_SUCCESS
#ifdef PLUGIN_DEF_AUTH
       || s1 == VPNCONNECT_PLUGIN_FUNC_DEFERRED
#endif
       ) && s2
#ifdef MANAGEMENT_DEF_AUTH
...
#ifdef PLUGIN_DEF_AUTH
      if (s1 == VPNCONNECT_PLUGIN_FUNC_DEFERRED)
