...
/*
 * VPNConnect.s default port number as assigned by IANA.
 */
#define VPNCONNECT_PORT "1194"
...
/* VPNConnect.sockaddr struct */
struct vpnconnect_sockaddr
{
...
struct link_socket_actual
{
  ...
  struct vpnconnect_sockaddr dest;
...
/*
 * This is the main socket structure used by VPNConnect.  The SOCKET_
...
#define vpnconnect_close_socket(s) closesocket(s)
...
#define vpnconnect_close_socket(s) close(s)
...
int vpnconnect_connect (socket_descriptor_t sd,
...
static inline
const char *print_vpnconnect_sockaddr_ex (const struct vpnconnect_sockaddr *addr,
			       const char* separator,
			       const unsigned int flags,
			       struct gc_arena *gc)
{
...
static inline
const char *print_vpnconnect_sockaddr (const struct vpnconnect_sockaddr *addr,
			    struct gc_arena *gc)
{
...
void setenv_sockaddr (struct env_set *es,
		      ...
		      const struct vpnconnect_sockaddr *addr,
		      const unsigned int flags);
...
/* return values of vpnconnect_inet_aton */
#define OIA_HOSTNAME   0
#define OIA_IP         1
#define OIA_ERROR     -1
int vpnconnect_inet_aton (const char *dotted_quad, struct in_addr *addr);
...
int vpnconnect_getaddrinfo (unsigned int flags,
                         const char *hostname,
                         const char *servname,
                         int resolve_retry_seconds,
                         volatile int *signal_received,
...
static inline bool
addr_defined (const struct vpnconnect_sockaddr *addr)
{
  if (!addr) return 0;
...
static inline bool
addr_match (const struct vpnconnect_sockaddr *a1, const struct vpnconnect_sockaddr *a2)
{
...
static inline bool
addrlist_match (const struct vpnconnect_sockaddr *a1, const struct addrinfo *addrlist)
{
...
addr_host (const struct vpnconnect_sockaddr *addr)
{
  /* 
...
static inline bool
addrlist_port_match (const struct vpnconnect_sockaddr *a1, const struct addrinfo *a2)
{
  const struct addrinfo *curele;
...
static inline bool
addr_port_match (const struct vpnconnect_sockaddr *a1, const struct vpnconnect_sockaddr *a2)
{
  switch(a1->addr.sa.sa_family) {
    case AF_INET:
      return a1->addr.in4.sin_addr.s_addr == a2->addr.in4.sin_addr.s_addr
...
static inline bool
addr_match_proto (const struct vpnconnect_sockaddr *a1,
		  const struct vpnconnect_sockaddr *a2,
		  const int proto)
{
...
static inline bool
addrlist_match_proto (const struct vpnconnect_sockaddr *a1,
...
static inline void
addr_zero_host(struct vpnconnect_sockaddr *addr)
{
   switch(addr->addr.sa.sa_family) {
     case AF_INET:
...
static inline void
addr_copy_sa(struct vpnconnect_sockaddr *dst, const struct vpnconnect_sockaddr *src)
{
  dst->addr = src->addr;
}
...
link_socket_extract_tos (struct link_socket *ls, const struct buffer *ipbuf)
{
  if (ls && ipbuf)
    {
      struct vpnconnect_iphdr *iph = (struct vpnconnect_iphdr *) BPTR (ipbuf);
      ls->ptos = iph->tos;
