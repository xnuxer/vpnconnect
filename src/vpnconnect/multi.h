 ...
 *
 * This structure is used by VPNConnect.processes running in server-mode to
 ...
 * page describes the role the structure plays when VPNConnect.is running in
 * server-mode.
 */
...
/**
 * Main VPNConnect.server state structure.
 *
 * This structure is used by VPNConnect.processes running in server-mode to
...
# define MC_WORK_THREAD                (MC_MULTI_THREADED_WORKER|MC_MULTI_THREADED_SCHEDULER)
  ...
  struct mbuf_set *mbuf;        /**< Set of buffers for passing data
                                 ...
                                 *   instances. */
  struct multi_tcp *mtcp;       /**< State specific to VPNConnect.using TCP
...
/**************************************************************************/
/**
 * Main event loop for VPNConnect.in server mode.
 * @ingroup eventloop
 *
 ...
 *
 * @note This function is only used by VPNConnect.processes which are running
...
multi_get_timeout (struct multi_context *m, struct timeval *dest)
{
  ...
  if (m->earliest_wakeup)
    {
      ASSERT (!vpnconnect_gettimeofday (&current, NULL));
