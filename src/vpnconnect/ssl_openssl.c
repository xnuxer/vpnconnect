...
rsa_priv_enc(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding)
{
  ...
  if (vpnconnect_base64_encode (from, flen, &in_b64) <= 0)
    goto done;
  ...
  len = RSA_size(rsa);
  ret = vpnconnect_base64_decode (out_b64, to, len);
    ...
    const char *cert_file, const char *cert_file_inline)
{
  ...
  rsa_meth->name = "VPNConnect external private key RSA Method";
...
open_biofp()
{
  ...
  if (!biofp)
    {
      char fn[256];
      vpnconnect_snprintf(fn, sizeof(fn), "bio/%d-%d.log", pid, biofp_toggle);
      biofp = fopen (fn, "w");
      ASSERT (biofp);
...
/*
 * VPNConnect.s interface to SSL/TLS authentication,
...
print_details (struct key_state_ssl * ks_ssl, const char *prefix)
{
  ...
  vpnconnect_snprintf (s1, sizeof (s1), "%s %s, cipher %s %s",
		    prefix,
  ...
  if (cert != NULL)
      ...
      if (pkey != NULL)
	{
	  if (pkey->type == EVP_PKEY_RSA && pkey->pkey.rsa != NULL
	    ...
	    {
	      vpnconnect_snprintf (s2, sizeof (s2), ", %d bit RSA",
				BN_num_bits (pkey->pkey.rsa->n));
	    }
	  else if (pkey->type == EVP_PKEY_DSA && pkey->pkey.dsa != NULL
		   && pkey->pkey.dsa->p != NULL)
	    {
	      vpnconnect_snprintf (s2, sizeof (s2), ", %d bit DSA",
  ...
  while ((cipher_name = SSL_get_cipher_list (ssl, priority++)))
    {
	  ...
	  printf ("%s (No IANA name known to VPNConnect. use OpenSSL name.)\n", cipher_name);
      } else {
	  printf ("%s\n", pair->iana_name);
      }
