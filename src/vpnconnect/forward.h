 ...
 * will be sent through a VPN tunnel to a remote VPNConnect.peer.  It's
 ...
 *   - Call \c vpnconnect_encrypt() to encrypt and HMAC signed the packet.
 *   - Call \c tls_post_encrypt() to prepend the one-byte VPNConnect.header
 ...
 *
 * VPNConnect.running as client or as UDP server only has a single external
 * network socket, so this function can be called with the single (client
 * mode) or top level (UDP server) context as its argument. VPNConnect
 ...
 * - Call \c vpnconnect_decrypt() of the \link data_crypto Data Channel
