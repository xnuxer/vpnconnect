 ...
 * (for example) the VPNConnect.server LAN is running on 192.168.1.x, but then
 * an VPNConnect.client tries to connect from a public location that is also running
	     ...
	     const struct env_set *es)
{
  ...
  if (management)
	...
	argv_printf (&argv,
	  ...
	  vpnconnect_execve_check (&argv, es, S_FATAL, "Linux ip link set failed");
	...
	if (tun) {
		...
		argv_printf (&argv,
		  ...
		  argv_msg (M_INFO, &argv);
		  vpnconnect_execve_check (&argv, es, S_FATAL, "Linux ip addr add failed");
	} else {
		argv_printf (&argv,
      ...
      if ( do_ipv6 )
	{
	  ...
	  argv_msg (M_INFO, &argv);
	  vpnconnect_execve_check (&argv, es, S_FATAL, "Linux ip -6 addr add failed");
	}
	...
	argv_printf (&argv,
      ...
      argv_msg (M_INFO, &argv);
      vpnconnect_execve_check (&argv, es, S_FATAL, "Linux ifconfig failed");
      if ( do_ipv6 )
	{
	  argv_printf (&argv,
			  "%s %s add %s/%d",
	  ...
	  argv_msg (M_INFO, &argv);
	  vpnconnect_execve_check (&argv, es, S_FATAL, "Linux ifconfig inet6 failed");
	}
      tt->did_ifconfig = true;
      ...
      if (tun)
	{
	  ...
	  argv_msg (M_INFO, &argv);
	  if (!vpnconnect_execve_check (&argv, es, 0, "Solaris ifconfig phase-1 failed"))
        ...
        if (tt->topology == TOP_SUBNET)
	{
      ...
      argv_msg (M_INFO, &argv);
      if (!vpnconnect_execve_check (&argv, es, 0, "Solaris ifconfig phase-2 failed"))
	solaris_error_close (tt, es, actual, false);
	  ...
	  argv_msg (M_INFO, &argv);
	  vpnconnect_execve_check (&argv, es, 0, NULL);
	  ...
	  if ( tt->type == DEV_TYPE_TUN )
	   {
	  ...
	  else						/* tap mode */
	    {
	      ...
	      argv_printf (&argv, "%s %s inet6 plumb up",
			    IFCONFIG_PATH, actual );
	      argv_msg (M_INFO, &argv);
	      if (!vpnconnect_execve_check (&argv, es, 0, "Solaris ifconfig IPv6 (prepare) failed"))
	  ...
	  argv_msg (M_INFO, &argv);
	  if (!vpnconnect_execve_check (&argv, es, 0, "Solaris ifconfig IPv6 failed"))
	    solaris_error_close (tt, es, actual, true);
        }
	...
	argv_printf (&argv,
      ...
      argv_msg (M_INFO, &argv);
      vpnconnect_execve_check (&argv, es, S_FATAL, "OpenBSD ifconfig failed");
      if ( do_ipv6 )
	{
	  argv_printf (&argv,
			  "%s %s inet6 %s/%d",
	  ...
	  argv_msg (M_INFO, &argv);
	  vpnconnect_execve_check (&argv, es, S_FATAL, "OpenBSD ifconfig inet6 failed");
...
# define NETBSD_MULTI_AF
	...
	if ( tt->topology == TOP_SUBNET )
	{
      ...
      argv_msg (M_INFO, &argv);
      vpnconnect_execve_check (&argv, es, S_FATAL, "NetBSD ifconfig failed");
      ...
      if ( do_ipv6 )
	{
	  ...
	  argv_msg (M_INFO, &argv);
	  vpnconnect_execve_check (&argv, es, S_FATAL, "NetBSD ifconfig inet6 failed");

	  /* and, hooray, we explicitely need to add a route... */
	  add_route_connected_v6_net(tt, es);
#else
	  msg( M_INFO, "no IPv6 support for tun interfaces on NetBSD before 4.0 (if your system is newer, recompile vpnconnect)" );
	  tt->ipv6 = false;
#endif
	}
      tt->did_ifconfig = true;
	...
	argv_printf (&argv,
      ...
      argv_msg (M_INFO, &argv);
      vpnconnect_execve_check (&argv, es, S_FATAL, "Mac OS X ifconfig failed");
      tt->did_ifconfig = true;
      ...
      if ( do_ipv6 )
	{
	  ...
	  argv_msg (M_INFO, &argv);
	  vpnconnect_execve_check (&argv, es, S_FATAL, "MacOS X ifconfig inet6 failed");
	...
	argv_printf (&argv,
      ...
      argv_msg (M_INFO, &argv);
      vpnconnect_execve_check (&argv, es, S_FATAL, "FreeBSD ifconfig failed");
      tt->did_ifconfig = true;
      ...
      if ( do_ipv6 )
	{
	  ...
	  argv_msg (M_INFO, &argv);
	  vpnconnect_execve_check (&argv, es, S_FATAL, "FreeBSD ifconfig inet6 failed");
	}
	...
	argv_msg (M_INFO, &argv);
	vpnconnect_execve_check (&argv, aix_es, S_FATAL, "AIX ifconfig failed");
	tt->did_ifconfig = true;
	...
	if ( do_ipv6 )
	  {
	    ...
	    argv_msg (M_INFO, &argv);
	    vpnconnect_execve_check (&argv, aix_es, S_FATAL, "AIX ifconfig inet6 failed");
	  }
	...
	vpnconnect_snprintf(iface, sizeof(iface), "interface=%lu", idx);

	/* on windows, VPNConnect.does ifconfig first, open_tun later, so
		  ...
		  struct tuntap *tt)
{
  ...
  if (tt->type == DEV_TYPE_NULL)
  ...
  else
    {
      ...
      if (dev_node)
	{
	  vpnconnect_snprintf (tunname, sizeof (tunname), "%s", dev_node);
	}
      else
	{
	  /*
	  ...
	  if ( dynamic && strcmp( dev, "tap" ) == 0 )
	      ...
	      if ( ioctl( tt->fd, TAPGIFNAME, (void*)&ifr ) < 0 )
		{
	      ...
	      dynamic_opened = true;
	      vpnconnect_snprintf (tunname, sizeof (tunname), "/dev/%s", dynamic_name );
	    }
	  else
#endif
	  ...
	  if (dynamic && !has_digit((unsigned char *)dev))
	    {
	      ...
	      for (i = 0; i < 256; ++i)
		{
		  vpnconnect_snprintf (tunname, sizeof (tunname),
				    "/dev/%s%d", dev, i);
		  vpnconnect_snprintf (dynamic_name, sizeof (dynamic_name),
				    "%s%d", dev, i);
	   ...
	   */
	    ...
	    {
	      vpnconnect_snprintf (tunname, sizeof (tunname), "/dev/%s", dev);
	    }
	}
	...
	if (tt->type != DEV_TYPE_NULL && tt->did_ifconfig)
	  {
	    ...
	    argv_msg (M_INFO, &argv);
	    vpnconnect_execve_check (&argv, NULL, 0, "Linux ip addr del failed");
            ...
            if (tt->ipv6 && tt->did_ifconfig_ipv6_setup)
              {
                ...
                argv_msg (M_INFO, &argv);
                vpnconnect_execve_check (&argv, NULL, 0, "Linux ip -6 addr del failed");
#else
                argv_printf (&argv,
                            "%s %s del %s/%d",
                            IFCONFIG_PATH,
                ...
                argv_msg (M_INFO, &argv);
                vpnconnect_execve_check (&argv, NULL, 0, "Linux ifconfig inet6 del failed");
#endif
              }
  ...
  if (tt->ipv6)
    {
      ...
      if(iph->version == 6)
	pi.proto = htons(VPNCONNECT_ETH_P_IPV6);
      else
	pi.proto = htons(VPNCONNECT_ETH_P_IPV4);
...
open_tun (const char *dev, const char *dev_type, const char *dev_node, struct tuntap *tt)
{
  ...
  vpnconnect_snprintf (tt->actual_name, 32, "%s%d", dev_tuntap_type, ppa);
...
solaris_close_tun (struct tuntap *tt)
{
  if (tt)
      ...
      if ( tt->ipv6 && tt->did_ifconfig_ipv6_setup )
	{
	  ...
	  argv_msg (M_INFO, &argv);
	  vpnconnect_execve_check (&argv, NULL, 0, "Solaris ifconfig inet6 unplumb failed");
	  argv_reset (&argv);
	}
                     ...
                     const char *actual, bool unplumb_inet6 )
{
  ...
  if (unplumb_inet6)
    {
      argv_printf( &argv, "%s %s inet6 unplumb",
		   IFCONFIG_PATH, actual );
      argv_msg (M_INFO, &argv);
      vpnconnect_execve_check (&argv, es, 0, "Solaris ifconfig inet6 unplumb failed");
    }
  ...
  argv_msg (M_INFO, &argv);
  vpnconnect_execve_check (&argv, es, 0, "Solaris ifconfig unplumb failed");
  close_tun (tt);
...
close_tun (struct tuntap* tt)
{
  ...
  if (tt && (tt->type == DEV_TYPE_TUN || tt->persistent_if ) )
  ...
  else if (tt)
    {
      ...
      argv_msg (M_INFO, &argv);
      vpnconnect_execve_check (&argv, NULL, 0, "OpenBSD 'destroy tun interface' failed (non-critical)");
...
/* the current way VPNConnect.handles tun devices on NetBSD leads to
...
close_tun (struct tuntap *tt)
{
  ...
  if (tt && ( tt->type != DEV_TYPE_TUN || tt->persistent_if ) )
  ...
  else if (tt)
    {
      ...
      argv_msg (M_INFO, &argv);
      vpnconnect_execve_check (&argv, NULL, 0, "NetBSD 'destroy tun interface' failed (non-critical)");
  ...
  if (tt->type == DEV_TYPE_TUN)
    {
      u_int32_t type;
      struct iovec iv[2];
      struct vpnconnect_iphdr *iph;

      iph = (struct vpnconnect_iphdr *) buf;

      if (tt->ipv6 && VPNCONNECT_IPH_GET_VER(iph->version_len) == 6)
        type = htonl (AF_INET6);
      else 
        type = htonl (AF_INET);
 ...
 *
 * so, to avoid lingering tun/tap interfaces after VPNConnect.quits,
...
close_tun (struct tuntap *tt)
{
  if (tt && tt->persistent_if )		/* keep pre-existing if around */
  ...
  else if (tt)				/* close and destroy */
    {
      ...
      argv_msg (M_INFO, &argv);
      vpnconnect_execve_check (&argv, NULL, 0, "FreeBSD 'destroy tun interface' failed (non-critical)");
...
close_tun (struct tuntap* tt)
{
  if (tt)
      ...
      if ( tt->ipv6 && tt->did_ifconfig_ipv6_setup )
	{
	  ...
	  argv_msg (M_INFO, &argv);
	  vpnconnect_execve_check (&argv, NULL, 0, "MacOS X 'remove inet6 route' failed (non-critical)");
	}
...
open_tun (const char *dev, const char *dev_type, const char *dev_node, struct tuntap *tt)
{
  ...
  if ( strcmp( dev, "tap" ) == 0 )		/* find first free tap dev */
    {						/* (= no /dev/tapN node) */
      ...
      for (i=0; i<99; i++ )
	{
	  vpnconnect_snprintf (tunname, sizeof (tunname), "/dev/tap%d", i);
	...
	msg( M_FATAL, "cannot find unused tap device" );
      ...
      vpnconnect_snprintf( dynamic_name, sizeof(dynamic_name), "tap%d", i );
      dev = dynamic_name;
    }
  ...
  if ( access( tunname, F_OK ) < 0 && errno == ENOENT )
    {
      ...
      vpnconnect_execve_check (&argv, es, S_FATAL, "AIX 'create tun interface' failed");
      env_set_destroy (es);
    }
  else
    {
...
close_tun (struct tuntap* tt)
{
  ...
  argv_msg (M_INFO, &argv);
  env_set_add( es, "ODMDIR=/etc/objrepos" );
  vpnconnect_execve_check (&argv, es, 0, "AIX 'destroy tap interface' failed (non-critical)");
...
get_tap_reg (struct gc_arena *gc)
{
  ...
  while (true)
	...
	msg (M_FATAL, "Error enumerating registry subkeys of key: %s",
      ...
      vpnconnect_snprintf (unit_string, sizeof(unit_string), "%s\\%s",
...
get_panel_reg (struct gc_arena *gc)
{
  ...
  while (true)
	...
	msg (M_FATAL, "Error enumerating registry subkeys of key: %s",
      ...
      vpnconnect_snprintf (connection_string, sizeof(connection_string),
			"%s\\%s\\Connection",
...
tap_allow_nonadmin_access (const char *dev_node)
{
  ...
  if (dev_node)
	...
	msg (M_FATAL, "TAP-Windows adapter '%s' not found", dev_node);
      ...
      vpnconnect_snprintf (device_path, sizeof(device_path), "%s%s%s",
			USERMODEDEVICEDIR,
			device_guid,
			TAP_WIN_SUFFIX);
  ...
  for (i = 0; i < n; ++i)
    {
      ...
      status = vpnconnect_execve_check (a, NULL, 0, "ERROR: netsh command failed");
...
ipconfig_register_dns (const struct env_set *es)
{
  ...
  status = vpnconnect_execve_check (&argv, es, 0, err);
  argv_reset(&argv);
  ...
  if (tt->options.dhcp_pre_release || tt->options.dhcp_renew)
    {
      ...
      buf_printf (&cmd, "vpnconnect --verb %d --tap-sleep %d", verb, pre_sleep);
  ...
  if (tt && tt->options.register_dns)
    {
      ...
      buf_printf (&cmd, "vpnconnect --verb %d --register-dns --rdns-internal", verb);
      fork_to_self (BSTR (&cmd));
      gc_free (&gc);
    }
}
