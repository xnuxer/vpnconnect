...
#endif
{
  ...
  buf.len = 0;
#ifdef DMALLOC
  buf.data = vpnconnect_dmalloc (file, line, size);
#else
...
#endif
{
  ...
  ret.len = buf->len;
#ifdef DMALLOC
  ret.data = (uint8_t *) vpnconnect_dmalloc (file, line, buf->capacity);
#else
 ...
 *
 * This functionality is duplicated in src/vpnconnectserv/common.c
 * Any modifications here should be done to the other place as well.
 */

bool vpnconnect_snprintf(char *str, size_t size, const char *format, ...)
{
  va_list arglist;
  int len = -1;
  if (size > 0)
  ...
  if (a)
    {
      struct gc_entry *e;
#ifdef DMALLOC
      e = (struct gc_entry *) vpnconnect_dmalloc (file, line, size + sizeof (struct gc_entry));
#else
    ...
    {
#ifdef DMALLOC
      ret = vpnconnect_dmalloc (file, line, size);
#else
      ret = malloc (size);
#endif
...
void gc_addspecial (void *addr, void (free_function)(void*), struct gc_arena *a)
{
  ...
  struct gc_entry_special *e;
#ifdef DMALLOC
  e = (struct gc_entry_special *) vpnconnect_dmalloc (file, line, sizeof (struct gc_entry_special));
#else
  ...
  if (str)
    {
      ...
      if (gc) {
         ...
         */
#ifdef DMALLOC
        ret = vpnconnect_dmalloc (file, line, n);
        memset(ret, 0, n);
#else
        ret = calloc(1, n);
#endif
