...
struct key_ctx
{
  ...
  uint8_t implicit_iv[VPNCONNECT_MAX_IV_LENGTH];
...
struct crypto_options
{
  ...
  struct packet_id_persist *pid_persist;
                                 ...
                                 *   VPNConnect.process startups. */

# define CO_PACKET_ID_LONG_FORM  (1<<0)
                                /**< Bit-flag indicating whether to use
                                 *   VPNConnect.s long packet ID format. */
 ...
 */
#define VPNCONNECT_AEAD_MIN_IV_LEN (sizeof (packet_id_type) + 8)
 ...
 * VPN tunnel packet to a remote VPNConnect.peer.
 * @ingroup data_crypto
 *
 * This function handles encryption and HMAC signing of a data channel
 * packet before it is sent to its remote VPNConnect.peer.  It receives the
 ...
 *     error occurred.
 */
void vpnconnect_encrypt (struct buffer *buf, struct buffer work,
 ...
 * packet received from a remote VPNConnect.peer.  It receives the necessary
 ...
 *                       remote VPNConnect.peer on which to perform security
 *                       operations.
 ...
 *     an error occurred.
 */
bool vpnconnect_decrypt (struct buffer *buf, struct buffer work,
