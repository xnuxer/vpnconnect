 ...
 *  Copyright (C) 2010 Fox Crypto B.V. <vpnconnect@fox-it.com>
 *
...
static void
vpnconnect_encrypt_aead (struct buffer *buf, struct buffer work,
  ...
  /* Prepare IV */
  {
    ...
    uint8_t iv[VPNCONNECT_MAX_IV_LENGTH];
    const int iv_len = cipher_ctx_iv_length (ctx->cipher);

    ASSERT (iv_len >= VPNCONNECT_AEAD_MIN_IV_LEN && iv_len <= VPNCONNECT_MAX_IV_LENGTH);
...
static void
vpnconnect_encrypt_v1 (struct buffer *buf, struct buffer work,
		 struct crypto_options *opt)
{
  struct gc_arena gc;
  gc_init (&gc);
  ...
  if (buf->len > 0 && opt)
    {
      ...
      if (ctx->cipher)
	{
	  uint8_t iv_buf[VPNCONNECT_MAX_IV_LENGTH];
	  ...
	  ASSERT (cipher_kt_mode (cipher_kt) != VPNCONNECT_MODE_CBC ||
	      outlen == iv_size);
	}
      else				/* No Encryption */
	{
...
void
vpnconnect_encrypt (struct buffer *buf, struct buffer work,
		 struct crypto_options *opt)
{
  if (buf->len > 0 && opt)
    {
	...
	vpnconnect_encrypt_aead(buf, work, opt);
      else
	vpnconnect_encrypt_v1(buf, work, opt);
    }
}
 ...
 */
static bool
vpnconnect_decrypt_aead (struct buffer *buf, struct buffer work,
    ...
    const uint8_t *ad_start)
{
  ...
  {
    uint8_t iv[VPNCONNECT_MAX_IV_LENGTH] = { 0 };
    const int iv_len = cipher_ctx_iv_length (ctx->cipher);
 ...
 */
static bool
vpnconnect_decrypt_v1 (struct buffer *buf, struct buffer work,
		 struct crypto_options *opt, const struct frame* frame)
{
...
bool
vpnconnect_decrypt (struct buffer *buf, struct buffer work,
		 struct crypto_options *opt, const struct frame* frame,
		 const uint8_t *ad_start)
{
  bool ret = false;
  ...
  if (buf->len > 0 && opt)
	...
	{
	  ret = vpnconnect_decrypt_aead (buf, work, opt, frame, ad_start);
	}
      else
	{
	  ret = vpnconnect_decrypt_v1 (buf, work, opt, frame);
	}
    }
  else
    {
	       ...
	       bool tls_mode, bool warn)
{
  ...
  if (ciphername && ciphername_defined)
    {
      kt->cipher = cipher_kt_get (translate_cipher_name_from_vpnconnect(ciphername));
  ...
  if (kt->cipher && kt->cipher_length > 0)
    {
      ...
      msg (D_HANDSHAKE, "%s: Cipher '%s' initialized with %d bit key",
          prefix,
          translate_cipher_name_to_vpnconnect(cipher_kt_name(kt->cipher)),
          kt->cipher_length *8);
...
test_crypto (struct crypto_options *co, struct frame* frame)
{
  ...
  /* init implicit IV */
  {
	...
	ASSERT (cipher_kt_iv_size(cipher) <= VPNCONNECT_MAX_IV_LENGTH);
	ASSERT (cipher_kt_iv_size(cipher) >= VPNCONNECT_AEAD_MIN_IV_LEN);

	/* Generate dummy implicit IV */
	ASSERT (rand_bytes(co->key_ctx_bi.encrypt.implicit_iv,
	    VPNCONNECT_MAX_IV_LENGTH));
	co->key_ctx_bi.encrypt.implicit_iv_len = impl_iv_len;

	memcpy(co->key_ctx_bi.decrypt.implicit_iv,
	    co->key_ctx_bi.encrypt.implicit_iv, VPNCONNECT_MAX_IV_LENGTH);
  ...
  for (i = 1; i <= TUN_MTU_SIZE (frame); ++i)
    {
      ...
      /* encrypt */
      vpnconnect_encrypt (&buf, encrypt_workspace, co);

      /* decrypt */
      vpnconnect_decrypt (&buf, decrypt_workspace, co, frame, BPTR (&buf));
	...
	if (key2.n != 2)
	  {
		...
		"have VPNConnect.Static Key format.  Using free-form passphrase "
      ...
      init_key_ctx (&ctx->encrypt, &key2.keys[kds.out_key], key_type, VPNCONNECT_OP_ENCRYPT,
		    "Outgoing Control Channel Authentication");
      init_key_ctx (&ctx->decrypt, &key2.keys[kds.in_key], key_type, VPNCONNECT_OP_DECRYPT,
...
static const char static_key_head[] = "-----BEGIN VPNConnect.Static key V1-----";
static const char static_key_foot[] = "-----END VPNConnect.Static key V1-----";
  ...
  for (; i < cipher_name_translation_table_count; i++)
    {
      ...
      if (0 == strcmp (cipher_name, pair->vpnconnect_name) ||
...
const char *
translate_cipher_name_from_vpnconnect (const char *cipher_name) {
...
const char *
translate_cipher_name_to_vpnconnect (const char *cipher_name) {
  const cipher_name_pair *pair = get_cipher_name_pair(cipher_name);

  if (NULL == pair)
    return cipher_name;

  return pair->vpnconnect_name;
}
