...
#ifndef LOG_VPNCONNECT
#define LOG_VPNCONNECT LOG_DAEMON
#endif
#endif
 ...
 *  (3) if VPNCONNECT_DEBUG_COMMAND_LINE is defined, output
...
/* If true, we forked from main VPNConnect.process */
...
error_reset ()
{
  ...
  mute_count = 0;
  mute_category = 0;
  default_out = VPNCONNECT_MSG_FP;
  default_err = VPNCONNECT_MSG_FP;

#ifdef VPNCONNECT_DEBUG_COMMAND_LINE
  msgfp = fopen (VPNCONNECT_DEBUG_FILE, "w");
  if (!msgfp)
    vpnconnect_exit (VPNCONNECT_EXIT_STATUS_CANNOT_OPEN_DEBUG_FILE); /* exit point */
#else
  msgfp = NULL;
#endif
}
...
{
  default_err = VPNCONNECT_ERROR_FP;  
}
...
void x_msg_va (const unsigned int flags, const char *format, va_list arglist)
{
  ...
  if ((flags & M_ERRNO) && e)
    {
      vpnconnect_snprintf (m2, ERR_BUF_SIZE, "%s: %s (errno=%d)",
  ...
  if (flags & M_OPTERR)
    {
      vpnconnect_snprintf (m2, ERR_BUF_SIZE, "Options error: %s", m1);
      SWAP;
    }
  ...
  if (!forked)
    {
      ...
      if (vo)
	{
	  vpnconnect_snprintf (m2, ERR_BUF_SIZE, "%s%s%s",
			    prefix,
			    prefix_sep,
			    m1);
...
open_syslog (const char *pgmname, bool stdio_to_null)
{
  ...
  if (!msgfp && !std_redir)
      ...
      if (!use_syslog)
	{
	  ...
	  openlog (pgmname_syslog, LOG_PID, LOG_VPNCONNECT);
	  use_syslog = true;
...
x_check_status (int status,
		...
		struct tuntap *tt)
{
  const int my_errno = vpnconnect_errno ();
...
void
vpnconnect_exit (const int status)
{
  if (!forked)
    {
      void tun_abort();
	...
	port_share_abort (port_share);
      ...
      if (status == VPNCONNECT_EXIT_STATUS_ERROR)
	abort ();
#endif

      if (status == VPNCONNECT_EXIT_STATUS_GOOD)
	perf_output_results ();
    }
