...
{
  int offset;
  const struct vpnconnect_iphdr *ih;
  ...
  if (tunnel_type == DEV_TYPE_TUN)
    ...
    {
      const struct vpnconnect_ethhdr *eh;
      if (BLEN (buf) < (int)(sizeof (struct vpnconnect_ethhdr)
	  + sizeof (struct vpnconnect_iphdr)))
	return false;
      eh = (const struct vpnconnect_ethhdr *) BPTR (buf);
      if (ntohs (eh->proto) != (ip_ver == 6 ? VPNCONNECT_ETH_P_IPV6 : VPNCONNECT_ETH_P_IPV4))
	return false;
      offset = sizeof (struct vpnconnect_ethhdr);
    }
  else
    return false;

  ih = (const struct vpnconnect_iphdr *) (BPTR (buf) + offset);

  /* IP version is stored in the same bits for IPv4 or IPv6 header */
  if (VPNCONNECT_IPH_GET_VER (ih->version_len) == ip_ver)
