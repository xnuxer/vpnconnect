                              ...
                              volatile int *signal_received)
{
  ...
  vpnconnect_snprintf (to_send, sizeof (to_send), "\x01%c%s%c%s", (int) strlen(creds.username),
...
recv_socks_reply (socket_descriptor_t sd,
		  struct vpnconnect_sockaddr *addr,
		  volatile int *signal_received)
{
  char atyp = '\0';
  int alen = 0;
...
establish_socks_proxy_passthru (struct socks_proxy_info *p,
			        ...
			        const char *servname,   /* vpnconnect server port */
			        volatile int *signal_received)
{
  char buf[128];
  size_t len;
...
establish_socks_proxy_udpassoc (struct socks_proxy_info *p,
				...
				struct vpnconnect_sockaddr *relay_addr,
			        volatile int *signal_received)
{
