...
/*
 * Win32-specific VPNConnect.code, targetted at the mingw
...
/*
 * Service functions for vpnconnect_execve
 */
...
env_block (const struct env_set *es)
{
  ...
  if (!vpnconnect_snprintf(force_path, sizeof(force_path), "PATH=%s\\System32;%s;%s\\System32\\Wbem",
...
vpnconnect_execve (const struct argv *a, const struct env_set *es, const unsigned int flags)
{
  ...
  if (a && a->argv[0])
          ...
          if (CreateProcessW (cmd, cl, NULL, NULL, FALSE, proc_flags, env, NULL, &start_info, &proc_info))
            {
              ...
              if (GetExitCodeProcess (proc_info.hProcess, &exit_status))
                ...
                msg (M_WARN|M_ERRNO, "vpnconnect_execve: GetExitCodeProcess %S failed", cmd);
              CloseHandle (proc_info.hProcess);
            }
          else
            {
              msg (M_WARN|M_ERRNO, "vpnconnect_execve: CreateProcess %S failed", cmd);
            }
          free (env);
          gc_free (&gc);
        }
    ...
    {
      msg (M_WARN, "vpnconnect_execve: called with empty argv");
    }
  return ret;
}
...
win_wfp_block_dns (const NET_IFINDEX index, const HANDLE msg_channel)
{
  ...
  status = GetModuleFileNameW (NULL, vpnconnectpath, sizeof(vpnconnectpath));
  if (status == 0 || status == sizeof(vpnconnectpath))
    {
      msg (M_WARN|M_ERRNO, "block_dns: cannot get executable path");
      goto out;
    }

  status = add_block_dns_filters (&m_hEngineHandle, index, vpnconnectpath,
