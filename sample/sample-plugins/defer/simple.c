...
vpnconnect_plugin_open_v1 (unsigned int *type_mask, const char *argv[], const char *envp[])
{
  ...
  printf ("FUNC: vpnconnect_plugin_open_v1\n");
  ...
  *type_mask =
    ...
    VPNCONNECT_PLUGIN_MASK (VPNCONNECT_PLUGIN_ENABLE_PF);
...
vpnconnect_plugin_func_v2 (vpnconnect_plugin_handle_t handle,
			const int type,
			const char *argv[],
			const char *envp[],
			...
			struct vpnconnect_plugin_string_list **return_list)
{
  ...
  switch (type)
    ...
    case VPNCONNECT_PLUGIN_ENABLE_PF:
      printf ("VPNCONNECT_PLUGIN_ENABLE_PF\n");
      if (context->test_packet_filter)
	return VPNCONNECT_PLUGIN_FUNC_SUCCESS;
      else
	return VPNCONNECT_PLUGIN_FUNC_ERROR;
...
{
  printf ("FUNC: vpnconnect_plugin_client_constructor_v1\n");
  return calloc (1, sizeof (struct plugin_per_client_context));
}

VPNCONNECT_EXPORT void
vpnconnect_plugin_client_destructor_v1 (vpnconnect_plugin_handle_t handle, void *per_client_context)
{
  printf ("FUNC: vpnconnect_plugin_client_destructor_v1\n");
...
{
  struct plugin_context *context = (struct plugin_context *) handle;
  printf ("FUNC: vpnconnect_plugin_close_v1\n");
  free (context);
}
