...
# A Sample VPNConnect.aware firewall.
...
# Allow incoming VPNConnect.packets
# Duplicate the line below for each
# VPNConnect.tunnel, changing --dport n
# to match the VPNConnect.UDP port.
#
# In VPNConnect. the port number is
...
# approach (see the VPNConnect.HOWTO),
# then comment out the line below.

iptables -A INPUT -p udp --dport 1194 -j ACCEPT

# Allow packets from TUN/TAP devices.
# When VPNConnect.is run in a secure mode,
