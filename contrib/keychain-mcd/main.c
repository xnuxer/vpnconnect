...
#include "../../src/vpnconnect/base64.h"
...
void handle_rsasign(FILE *man_file, SecIdentityRef identity, const char *input)
{
    ...
    input_len = vpnconnect_base64_decode(input_b64, input_binary, input_len);
    if (input_len < 0)
        errx(1, "vpnconnect_base64_decode: overflow");
    ...
    vpnconnect_base64_encode(output_binary, output_len, &output_b64);
...
void handle_needcertificate(FILE *man_file, SecIdentityRef identity)
{
    ...
    vpnconnect_base64_encode(cert, cert_len, &result_b64);
#if 0
...
void management_loop(SecIdentityRef identity, int man_fd, const char *password)
{
    ...
    while (1) {
        ...
        if (is_prefix(buffer, ">FATAL"))
            fprintf(stderr, "Fatal message from VPNConnect. %s\n", buffer+7);
        if (is_prefix(buffer, ">INFO"))
            fprintf(stderr, "INFO message from VPNConnect. %s\n", buffer+6);
    }
}
...
int main(int argc, char* argv[])
{
    ...
    fprintf(stderr, "Successfully connected to vpnconnect\n");
