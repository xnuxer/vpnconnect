 ...
 *
 * This section describes how VPNConnect.stores its VPN tunnel state during
 * operation.
 *
 * VPNConnect.uses several data structures as storage containers for state
 ...
 *
 * Whether an VPNConnect.process is running in client-mode or server-mode
 ...
 *
 * In the old v1.x series, an VPNConnect.process managed only one single VPN
 ...
 *
 * This changed, however, in the v2.x series, as new VPNConnect.versions
 ...
 * majority of these changes are only relevant for VPNConnect.processes
 ...
 *
 * An VPNConnect.process running in client-mode can manage at most one single
 ...
 * is then passed to \c tunnel_point_to_point() which drives VPNConnect.s
 ...
 *
 * An VPNConnect.process running in server-mode can manage multiple
 * simultaneously active VPN tunnels.  For every VPN tunnel active, in
 * other words for every VPNConnect.client which is connected to a server,
 * the VPNConnect.server has one \c context structure in which it stores that
 ...
 *
 * An VPNConnect.process running in server-mode starts in the same \c main()
 ...
 *
 * When an VPNConnect.client makes a new connection to a server, the server
