#! /bin/sh
#
# t_lpback.sh - script to test VPNConnect.s crypto loopback
...
# Get list of supported ciphers from vpnconnect --show-ciphers output
CIPHERS=$(${top_builddir}/src/vpnconnect/vpnconnect --show-ciphers | \
...
"${top_builddir}/src/vpnconnect/vpnconnect" --genkey --secret key.$$
set +e
...
do
    ...
    ( "${top_builddir}/src/vpnconnect/vpnconnect" --test-crypto --secret key.$$ --cipher ${cipher} ) >log.$$ 2>&1
    if [ $? != 0 ] ; then
        echo "FAILED"
        cat log.$$
        e=1
