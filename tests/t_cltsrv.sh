#! /bin/sh
#
# t_cltsrv.sh - script to test VPNConnect.s crypto loopback
...
for i in 1 2 3 ; do
  set +e
  (
  "${top_builddir}/src/vpnconnect/vpnconnect" --script-security 2 --cd "${root}" ${addopts} --setenv role srv --down "${downscript}" --tls-exit --ping-exit 180 --config "sample-config-files/loopback-server" &
  "${top_builddir}/src/vpnconnect/vpnconnect" --script-security 2 --cd "${top_srcdir}/sample" ${addopts} --setenv role clt --down "${downscript}" --tls-exit --ping-exit 180 --config "sample-config-files/loopback-client"
